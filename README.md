...
- **Family calendar wall** — day, week, and agenda views with person colors, school layers, homework, uniforms, dinners, and groceries
...
- **Google Calendar** — shared schedules, school, homework, and RSVPs with your household's OAuth
- **OpenAI / Codex OAuth** — optional ChatGPT/Codex device login with native app-server tool calling
- **Grok OAuth** — coming soon; not part of the public setup yet
- **Reminders & automations** — per-person DMs or channel pings
- **Meal planning** — dinners and groceries on the calendar wall, plus the dedicated family channel
...
| **Bernie holds the keys** | Your LLM provider (Anthropic, OpenAI, OpenRouter, Ollama, …), HA, and Google credentials live in one place; clients get a single bearer token over an OpenAI-compatible API |
...
| **Brain** | Your pick — Anthropic, OpenRouter, optional ChatGPT/Codex OAuth, local Ollama, … |
| **Schedule** | Google Calendar OAuth → family day/week/agenda wall, school, homework, dinners, and groceries |
| **Home** | Home Assistant → Frigate, UniFi, sensors, vehicles, sleep |
| **Data** | SQLite on your host; `.env` + `credentials/` never in git |

---

## Model providers (optional Codex OAuth)

Bernie is **provider-agnostic** — pick any supported route for everyday chat:

- **Anthropic**, **OpenRouter** (including low-cost models such as GPT-5.6 Luna), **LiteLLM/proxy**, or **local Ollama**
- **OpenAI via ChatGPT/Codex OAuth** (optional) runs through an isolated subscription runner with native Codex app-server tool calling
- **Grok OAuth is coming soon** and is not a supported public setup yet

Optional Codex OAuth can pin an OpenRouter fallback for continuity if the subscription needs reauthentication. Activity keeps provider usage separate; subscription attempts also remain visible as provider-labelled Langfuse observations.

### Authenticate OpenAI with ChatGPT/Codex OAuth

This uses the official Codex device login and stores OAuth state in the runner's Docker volume—no OpenAI API key or host `~/.codex` mount. Generate `SUBSCRIPTION_RUNNER_SECRET` in `.env` first.

```bash
# From the Bernie repository on the Docker host
openssl rand -hex 32   # save as SUBSCRIPTION_RUNNER_SECRET in .env
docker compose --profile subscriptions up -d --build bernie-subscription-runner

# Open the printed URL in any browser and enter the device code
docker compose --profile subscriptions run --rm \
  --entrypoint codex bernie-subscription-runner login --device-auth

# Verify the volume-backed login
docker compose --profile subscriptions run --rm \
  --entrypoint codex bernie-subscription-runner login status
```

Then enable the `provider: "codex"` entry in `subscription_models` and select its model (for example `gpt-5.6-luna`) in Bernie's model settings. Re-run the same device-login command whenever provider status reports `reauth-required`. Never paste the device code or OAuth files into Discord, logs, or `credentials/`.

For direct Luna setup, Codex OAuth configuration, and readiness details: [**OpenAI models and Codex OAuth**](docs/subscription-providers.md).

---

## Family calendar wall

Open the dashboard at `http://<host>:8000`, sign in, and choose **Calendar** in the family navigation:

- **Day** — one-day timeline with all-day events and current-time marker.
- **Week** — seven-day family wall with person colors and daily event stacks.
- **Agenda** — chronological seven-day list for a compact phone-friendly view.
- **People and school layers** — filter or dim by family member; parents/admins can show or hide school, homework, and uniform notes.
- **Dinner, groceries, and chores** — edit dinner from the day, see homework and chores beside the schedule, and open the shared grocery panel.

Arrow controls move the visible period; **Today** jumps back to the current date. The selected view and person filter last for the browser session. Calendar events come from the household's Google OAuth calendars; meals and groceries stay in Bernie's database, with optional dinner mirroring to Google Calendar.

Dashboard guide: [**Web dashboard**](docs/user-guide/web-dashboard.md) · calendar setup: [**Google OAuth**](docs/google-oauth.md).
...
**He keeps the keys.** Your model provider (Anthropic, OpenAI, OpenRouter, Ollama, LiteLLM, …) plus HA and Google live in Bernie's `.env` / `config.json`. Callers only need:
...
**You need:** Docker Compose, a Discord server, and **one** LLM path (API key, local Ollama, or optional Codex OAuth) — about 20 minutes.
...
$EDITOR .env          # DISCORD_TOKEN, INTERNAL_POST_SECRET, + one LLM path
$EDITOR config.json   # Discord snowflakes, timezone, family, active_model (match your LLM)
```
...
Hour-one is provider-agnostic: set whatever credentials match `active_model` (Anthropic, OpenRouter, Ollama, …).
...
| [**OpenAI models and Codex OAuth**](docs/subscription-providers.md) | Luna direct routing and optional ChatGPT/Codex OAuth |
| [**Web dashboard**](docs/user-guide/web-dashboard.md) | Today, family calendar wall, Home, Family, Plan, Activity |
