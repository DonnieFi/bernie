...
_WORKER_SYSTEM = (
    "You are Bernie, a helpful home assistant bot for the family. "
...
async def call_worker_model(topic: str) -> str | None:
    ...
    if is_subscription_enabled(worker_model, config):
        log.info("worker: routing to subscription chain model=%s", worker_model)
        from subscription_invoke import complete_text
        result = await complete_text(
            worker_model, config=config, prompt=topic, system=_WORKER_SYSTEM, surface="worker",
        )
        if result:
            return result
        log.warning("worker: subscription %s failed — chain already tried Ollama tier", worker_model)
        return None
...
async def call_ollama_fallback(topic: str, config: dict) -> str | None:
    ...
    if not ollama_models:
        ...
        return None
    fallback = (config.get("llm_fallback") or {}).get("model") or ollama_models[0]
