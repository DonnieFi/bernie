...
_file_handler_err: str | None = None
try:
    os.makedirs(os.path.dirname(BOT_LOG), exist_ok=True)
    _handlers.append(logging.FileHandler(BOT_LOG))
except OSError as exc:
    # discord container mounts ./data:ro — bind ./data/bot.log for shared audit log.
    _file_handler_err = str(exc)
...
if _file_handler_err:
    log.warning(
        "FileHandler unavailable for %s (%s) — logging to stderr only",
        BOT_LOG,
        _file_handler_err,
    )
