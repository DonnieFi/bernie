...
async def get_anthropic_spend_since(as_of: str) -> float:
    ...
    family-bot-joj.1: range on logged_at (not date(logged_at)) so idx_token_usage_logged_at works.
    """
    # as_of is calendar day; inclusive from start of that UTC day
    lower = f"{as_of}T00:00:00" if "T" not in as_of else as_of
    async with _db_read() as db:
        cur = await db.execute(
            """SELECT model, SUM(input_tokens), SUM(output_tokens),
                      SUM(cache_creation_tokens), SUM(cache_read_tokens)
               FROM token_usage
               WHERE model LIKE 'claude-%' AND logged_at >= ?
               GROUP BY model""",
            (lower,)
        )
        rows = await cur.fetchall()
...
async def get_or_spend(days: int = 30) -> float:
    ...
    family-bot-joj.1: range on logged_at (not date(logged_at)).
    """
    async with _db_read() as db:
        cur = await db.execute(
            """SELECT model, SUM(input_tokens), SUM(output_tokens),
