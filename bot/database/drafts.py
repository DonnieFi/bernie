...
async def get_unposted_drafts(limit: int = 100) -> list[dict]:
    # family-bot-t7h: bound draft fan-out (cursor not needed — small active set)
    lim = max(1, min(int(limit or 100), 500))
    async with _db_read() as db:
        cur = await db.execute(
            "SELECT * FROM pending_drafts WHERE posted=0 ORDER BY created_at ASC LIMIT ?",
            (lim,),
        )
