...
async def init_db():
    ...
    async with _db_conn() as db:
        ...
        await db.executescript("""
            ...
            -- family-bot-joj: channel + time for history tail / pruning
            CREATE INDEX IF NOT EXISTS idx_conversation_channel_created
                ON conversation_history(channel_id, created_at);
            CREATE INDEX IF NOT EXISTS idx_memory_events_person_logged
                ON memory_events(person_id, logged_at);
            ...
            -- family-bot-6wl: removed duplicate idx_token_usage_logged_at DDL
            ...
            -- family-bot-joj: claim sort (status, run_at, priority) + type filters
            CREATE INDEX IF NOT EXISTS idx_cognitive_tasks_claim
                ON cognitive_tasks(status, run_at, priority DESC, created_at);
            CREATE INDEX IF NOT EXISTS idx_cognitive_tasks_type_status
                ON cognitive_tasks(type, status, run_at);
...
async def ensure_meals_grocery_web_schema() -> None:
    """fqa9: groceries checked flag + meals GCal mirror id."""
    async with _db_conn() as db:
        cur = await db.execute("PRAGMA table_info(groceries)")
        grocery_cols = {row[1] for row in await cur.fetchall()}
        if "checked" not in grocery_cols:
            try:
                await db.execute(
                ...
                pass
        cur = await db.execute("PRAGMA table_info(meals)")
        meal_cols = {row[1] for row in await cur.fetchall()}
        if "gcal_event_id" not in meal_cols:
            try:
                await db.execute("ALTER TABLE meals ADD COLUMN gcal_event_id TEXT")
            except Exception:
                pass
        await db.commit()
