...
async def claim_next_task(
    *,
    ...
    family-bot-995: optional include/exclude type filters for light/heavy queues.
    """
    import json
    now = datetime.now(dt_timezone.utc).isoformat()
    clauses = ["status='queued'", "run_at <= ?"]
    params: list[Any] = [now]
    if include_types:
    ...
    async with _db_conn() as db:
        ...
        async with db.execute(
            f"""SELECT id FROM cognitive_tasks
               WHERE {where}
...
async def requeue_cognitive_task(task_id: int) -> bool:
    """Reset a failed/dead_letter task to queued (admin recovery). Returns True if updated."""
    now = datetime.now(dt_timezone.utc).isoformat()
    async with _db_conn() as db:
        cur = await db.execute(
            """UPDATE cognitive_tasks
               SET status='queued', retry_count=0, error=NULL, result=NULL,
                   started_at=NULL, completed_at=NULL, heartbeat=NULL, run_at=?
