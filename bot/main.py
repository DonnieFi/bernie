...
async def _common_setup(role: str = "monolith"):
    ...
    from completion_router import has_usable_llm_route

    anthropic_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    ok_route, route_reason = has_usable_llm_route(config)
    if not ok_route:
        logger.error(
            "No usable LLM route: set ANTHROPIC_API_KEY, OPENROUTER_API_KEY, "
            "a viable enabled subscription_models chain (with OpenRouter or Ollama "
            "fallback), or ollama_models. reason=%s",
            route_reason,
        )
        return {"token": None, "container": None, "server": None, "bot": None}
    if not anthropic_key:
        logger.warning(
            "ANTHROPIC_API_KEY unset — starting with route=%s only",
            route_reason,
        )
    else:
        logger.info("LLM startup route gate: %s", route_reason)

    # Warn when subscription primaries are enabled but the runner sidecar is down.
    try:
        from completion_router import subscription_catalog
        from subscription_complete import fetch_runner_health

        enabled_subs = [e for e in subscription_catalog(config) if e.enabled]
        if enabled_subs:
            if not (os.environ.get("SUBSCRIPTION_RUNNER_SECRET") or "").strip():
                logger.warning(
                    "subscription_models enabled but SUBSCRIPTION_RUNNER_SECRET unset — "
                    "runner POST /v1/completions rejects all requests (set in .env like INTERNAL_POST_SECRET)"
                )
            health = await fetch_runner_health(config)
            providers = (health or {}).get("providers") or {}
            if all(providers.get(k) == "unavailable" for k in ("codex", "grok")):
                logger.warning(
                    "subscription_models enabled but runner unreachable — "
                    "start with: docker compose --profile subscriptions up -d "
                    "bernie-subscription-runner (primary tier will fall back to OR/Ollama)"
                )
    except Exception:
        logger.debug("subscription runner startup probe failed", exc_info=True)
    ...
    litellm_key = os.environ.get("LTE_LLM_MASTER_KEY", "") or anthropic_key
    ...
    if litellm_key:
        ...
        logger.warning("LiteLLM client not constructed (no LTE_LLM_MASTER_KEY / Anthropic key)")
    ...
    ollama_url = configured_ollama_base_url(config)
    if ollama_url is None and config.get("ollama_models"):
        logger.warning(
            "ollama_models configured but ollama_base_url / ollama_base_urls "
            "is missing or still the placeholder — Ollama client not wired"
        )
    ...
    if role in ("discord", "monolith"):
        ...
        # family-bot-46t.1: after bg full presence fill, nudge open tabs to re-fetch
        container.presence.set_broadcaster(container.connection_manager.broadcast)
