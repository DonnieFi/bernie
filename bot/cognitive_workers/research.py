...
class ResearchWorker(CognitiveWorkerBase):
    ...
    async def handle(self, task: dict, container) -> dict:
        ...
        # Prefer upgrade_model for final synthesis when configured.
        synth_model = self.upgrade_model or self.default_model
        approx = len(synth_findings) // 4
        if self.upgrade_model and approx > self.escalate_above_tokens:
            synth_model = self.upgrade_model
        else:
            synth_model = self.default_model
        synth_prompt = (
