...
family-bot-995: light/heavy queues — heavy research cannot starve short family
jobs. Still one heavy at a time (single-writer safety); up to LIGHT_SLOTS lights
in parallel with the heavy.
"""
import asyncio
import logging
import time
...
# family-bot-995: multi-minute research / study / consolidation = heavy
HEAVY_TASK_TYPES = frozenset({
    "research",
    "study_guide",
    "consolidation",
})
# Concurrent light handlers. Keep ≤2; if write-lock spikes appear under load,
# drop LIGHT_SLOTS to 1 before any true multi-heavy concurrency (40A single-writer).
LIGHT_SLOTS = 2
...
class CognitiveWorker:
    """Polls cognitive_tasks every 10s; light/heavy queues (family-bot-995)."""

    def __init__(self, container):
        self.container = container
        self._loop: "tasks.Loop | None" = None
        self._heavy_running = False
        self._light_in_flight = 0
        self._bg_tasks: set[asyncio.Task] = set()
        # family-bot-995 polish: queue wait = run_at → handler start (seconds)
        self._last_queue_wait_s: float | None = None
        self._queue_wait_samples: list[float] = []  # ring of recent waits
        self._queue_wait_max = 50
    ...
    def _track(self, coro, *, heavy: bool) -> None:
        ...
        def _done(task: asyncio.Task) -> None:
            ...
            if heavy:
                ...
                self._light_in_flight = max(0, self._light_in_flight - 1)
            try:
                exc = task.exception()
            except asyncio.CancelledError:
                return
            if exc:
                log.exception("CognitiveWorker background task failed: %s", exc)
    ...
    async def _poll(self):
        ...
        try:
            # Prefer light work so research cannot starve family jobs
            while self._light_in_flight < LIGHT_SLOTS:
                light = await db.claim_next_task(exclude_types=tuple(HEAVY_TASK_TYPES))
                if not light:
                    break
                self._light_in_flight += 1
                log.info(
                    "CognitiveWorker: claimed LIGHT id=%d type=%s (in_flight=%d)",
                    light["id"], light["type"], self._light_in_flight,
                )
                self._track(self._run_task(light), heavy=False)

            if not self._heavy_running:
                heavy = await db.claim_next_task(include_types=tuple(HEAVY_TASK_TYPES))
                if heavy:
                    self._heavy_running = True
                    log.info(
                        "CognitiveWorker: claimed HEAVY id=%d type=%s",
                        heavy["id"], heavy["type"],
                    )
                    self._track(self._run_task(heavy), heavy=True)
        except Exception:
            log.exception("CognitiveWorker poll error")

    def queue_wait_stats(self) -> dict:
        """Admin/health: recent run_at → handler-start wait samples."""
        samples = list(self._queue_wait_samples)
        if not samples:
            return {
                "last_s": self._last_queue_wait_s,
                "count": 0,
                "p50_s": None,
                "p95_s": None,
                "max_s": None,
                "light_in_flight": self._light_in_flight,
    ...
    def _record_queue_wait(self, task: dict) -> float:
        """Seconds from task run_at to handler start (0 if run_at missing/unparseable)."""
        from datetime import datetime, timezone as dt_timezone

        run_at = task.get("run_at") or task.get("created_at")
        wait_s = 0.0
        if run_at:
            try:
                ...
                wait_s = max(0.0, (datetime.now(dt_timezone.utc) - dt).total_seconds())
            except Exception:
                wait_s = 0.0
        self._last_queue_wait_s = wait_s
        self._queue_wait_samples.append(wait_s)
        if len(self._queue_wait_samples) > self._queue_wait_max:
            self._queue_wait_samples = self._queue_wait_samples[-self._queue_wait_max :]
        return wait_s
    ...
    async def _run_task(self, task: dict) -> None:
        ...
        task_id = task["id"]
        task_type = task["type"]
        wait_s = self._record_queue_wait(task)
        if wait_s > 1.0:
            log.info(
                "CognitiveWorker: queue_wait=%.2fs id=%d type=%s (run_at→start)",
                wait_s, task_id, task_type,
            )
        ...
        try:
            ...
            log.info("CognitiveWorker: completed task id=%d type=%s", task_id, task_type)
            if _uid:
