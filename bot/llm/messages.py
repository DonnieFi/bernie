...
_TOOL_USE_STUB = "[tool call — pruned from history]"
        ...
        if isinstance(content, list):
            ...
            for block in content:
                ...
                if btype == "tool_result":
                    ...
                    # family-bot-jwf: prune long tool_use mid-session (not only results)
                    new_content.append({
                        "type": "text",
                        "text": f"{_TOOL_USE_STUB} ({block.get('name') or 'tool'})",
                    })
                else:
                    new_content.append(block)
