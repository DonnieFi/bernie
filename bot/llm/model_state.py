...
def _base_url_for_model(model_name: str) -> str | None:
    ...
    Subscription (Codex/Grok) models must never hit ``ServiceContainer.llm_for`` —
    they have no Anthropic/LiteLLM client. Use registry base URL (usually None).
    """
    try:
        from config import config as _cfg
        from model_registry import model_base_url, model_source
        if model_source(model_name, _cfg) in ("codex", "grok"):
            return model_base_url(model_name, _cfg)
    except Exception:
        pass
    if _container:
        try:
            ...
            # MissingProviderClient / subscription ValueError — no proxy URL.
            return None
