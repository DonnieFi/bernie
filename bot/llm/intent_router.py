...
def narrow_tool_domains_for_subscription(
    *,
    ...
    """Aggressive narrow for Grok/Codex tool loops (#anvil-sized surfaces).

    When intent is a single domain (calendar, email, …), skip the core bundle
    that causes multi-tool “evening update” sprawl. Otherwise fall back to
    ``narrow_tool_domains(..., force=True)``.
    """
    ...
    if len(matched) == 1:
        ...
        if ceiling is None or only in ceiling:
            return [only]
    return narrow_tool_domains(
...
def narrow_tool_domains(
    *,
    ...
    if not force and _channel_is(config, channel_id, "anvil"):
        return mode_domains
