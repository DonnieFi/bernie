...
# family-bot-9xg: tighten bare \beach\b / \bplan\b / lone multi-? that over-route family chat to smol
DEFAULT_SMOL_PATTERNS = [
    ...
    # aggregation — require noun after each / family scope after every|all|across
    r"\beach\s+(person|kid|child|member|of)\b",
    r"\bevery(one|body)\b.*\b(and|then|also|schedule|calendar)\b",
    r"\ball of (us|them|the)\b",
    r"\bacross\s+(the\s+)?(family|kids|household)\b",
    # planning — not bare "plan" (meal plan, plan for, plan around, make a plan)
    r"\b(make|work|figure)\s+(a\s+)?plan\b",
    r"\bplan\s+(out|around|for|the day)\b",
    r"\bplan\b.*\baround\b",
    r"\bfigure out\b", r"\bwork out\b",
...
def looks_multistep(user_message: str, app_config: dict) -> bool:
    ...
    if any(re.search(p, text) for p in patterns):
        return True
    # Two+ questions alone is weak ("huh??"); require a conjunction clause
    if text.count("?") >= 2 and re.search(r"\b(and|also|then|plus|or)\b", text):
        return True
    return False
