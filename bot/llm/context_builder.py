...
async def build_context(
    ...
    # family-bot-2wh.10 / family-bot-1j6: honor context.prefetch.ha (always|intent|never)
    # Default intent — full HA dump on every turn burns uncached tokens on chit-chat.
    ha_mode = str((ctx_cfg.get("prefetch") or {}).get("ha", "intent")).lower()
    if ha_mode == "never":
        do_ha = False
    elif ha_mode == "always":
        do_ha = True
    else:
        do_ha = looks_home_intent(user_message or "")  # intent (default)
