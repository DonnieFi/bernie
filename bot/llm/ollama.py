...
async def call_ollama(
    ...
    *,
    ...
    When ``skip_telemetry`` is True, do not write Langfuse/DB rows (subscription
    chain owns that via ``log_subscription_attempts``). In that mode the return
    value is ``(text, {"input_tokens": int|None, "output_tokens": int|None})``.
    """
    ...
    try:
        ...
        http_timeout = max(5, int(timeout_s)) if timeout_s is not None else 60
        ...
        async def _do_call(s: aiohttp.ClientSession):
            async with s.post(
                ...
                if resp.status != 200:
                    ...
                    err = "Sorry, I'm having trouble reaching both my primary and backup brains."
                    if skip_telemetry:
                        return err, {"input_tokens": None, "output_tokens": None}
                    return err
                data = await resp.json()
                content = data.get("message", {}).get("content", "") or (
                    "I'm offline right now (fallback returned empty)."
                )
                raw_in = data.get("prompt_eval_count")
                raw_out = data.get("eval_count")
                in_tok = int(raw_in) if isinstance(raw_in, (int, float)) and not isinstance(raw_in, bool) else None
                out_tok = int(raw_out) if isinstance(raw_out, (int, float)) and not isinstance(raw_out, bool) else None
                usage = {"input_tokens": in_tok, "output_tokens": out_tok}

                if not skip_telemetry:
                    # Never invent zeros: only persist when both counts are known.
        ...
        if skip_telemetry:
            # Subscription chain owns hop budget; queued_run clips at llm_step_timeout_s (~45s).
            return await _run()
        ...
        err = "Everything is offline. I'm sorry, I can't help right now."
        return (err, {"input_tokens": None, "output_tokens": None}) if skip_telemetry else err
    except RuntimeError as exc:
        if str(exc) == "shed":
            log.warning("Ollama call shed by llm queue")
            err = "Sorry, I'm busy right now — try again in a moment."
            return (err, {"input_tokens": None, "output_tokens": None}) if skip_telemetry else err
        raise
    except Exception as e:
