...
AUDIT_SYSTEM_PROMPT = (
    "You are Bernie, the household assistant. "
...
async def call_for_audit(draft: str, cfg: dict, container) -> str:
    ...
    if not audit_model:
        ...
        return draft
    if is_subscription_enabled(audit_model, cfg):
    ...
    client_or_url = container.llm_for(audit_model) if container else None
    ...
    if client is None:
        log.error("call_for_audit: no client for model=%s", audit_model)
        return draft
    try:
        # family-bot-1tv: route through LLM queue so nightly audit doesn't contend with family chat
        from llm.queue import queued_messages_create

        resp = await queued_messages_create(
            client,
            cfg,
            shadow=True,  # low-priority lane vs family chat
            model=audit_model,
            max_tokens=400,
            system=AUDIT_SYSTEM_PROMPT,
