...
async def _parse_caddy_ips(path: str) -> dict[str, set[str]]:
    """Extract literal IP upstreams from a Caddyfile (best-effort)."""
    result: dict[str, set[str]] = {}
    try:
        # ponytail: a timed-out NFS read leaves one worker thread; use a local
        # Caddyfile copy if this check needs to run reliably while NFS is down.
        text = await asyncio.wait_for(
            asyncio.to_thread(Path(path).read_text, encoding="utf-8", errors="replace"),
            timeout=5,
        )
...
async def poll() -> list[dict]:
    ...
    if caddy_path and await _caddy_check_due(interval_hours):
        events.extend(_check_caddy_stale(await _parse_caddy_ips(caddy_path), host_ips))
