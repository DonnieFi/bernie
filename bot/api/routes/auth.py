...
def build_auth_router(ctx: Any) -> APIRouter:
    ...
    async def get_me(user: _ac.Person = Depends(_ac.verify_token)):
        ...
        # family-bot-julyaudit P1: snapshot-first (same path as Today/Family)
        presence, _ = await presence_service.get_full_presence_for_web()
        if not isinstance(presence, dict):
            presence = {}
        me_row = presence.get(user.id) or {}
        if not me_row:
            # tolerate who/id aliases
            me_row = next(
                (
                    p for p in presence.values()
                    if isinstance(p, dict)
                    and (p.get("id") == user.id or p.get("who") == user.id)
                ),
                {},
            )
        
        # family-bot-co8: family_name + web favorites from config (no Example default)
        from web_ui_prefs import web_ui_prefs
        ui = web_ui_prefs(_ac.config if isinstance(_ac.config, dict) else {})
        return {
            "id": user.id,
            "name": real_name,
            "role": user.role,
            "family_name": ui.get("family_name") or "Family",
            "quick_lights": ui.get("quick_lights") or [],
            "air_quality_rooms": ui.get("air_quality_rooms"),
            "person_colors": ui.get("person_colors") or {},
            "preferences": prefs,
            "presence": me_row,
