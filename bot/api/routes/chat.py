...
def build_chat_router(ctx: Any) -> APIRouter:
    ...
    async def chat_endpoint(req: _ac.ChatRequest, auth_user: _ac.Person = Depends(_ac.verify_token)):
        ...
        try:
            try:
                web_model = _ac.config.get("webui_model")
                try:
                    from model_catalog import is_subscription_enabled
                    web_timeout = 180.0 if is_subscription_enabled(web_model, _ac.config) else 45.0
                except Exception:
                    web_timeout = 45.0
    ...
    async def openai_completions(req: OAICompletionRequest):
        ...
        try:
            openwebui_model = (
                _ac.config.get("openwebui_model") or _ac.config.get("webui_model")
            )
            try:
                from model_catalog import is_subscription_enabled
                openwebui_timeout = (
                    180.0 if is_subscription_enabled(openwebui_model, _ac.config) else 45.0
                )
            except Exception:
                openwebui_timeout = 45.0
            reply = await asyncio.wait_for(
                chat_general(
            ...
            raise HTTPException(
                status_code=504, detail="Bernie is thinking too hard (request timed out).",
            )
        except Exception as e:
