...
def build_activity_router(ctx: Any) -> APIRouter:
    ...
    async def get_keys_status():
        ...
        # family-bot-mb6: gather spend aggregates concurrently
        since = (datetime.now(timezone.utc) - timedelta(days=30)).date().isoformat()
        anth_spend_30d, usage_monthly, usage_weekly, usage_daily = await asyncio.gather(
            db.get_anthropic_spend_since(since),
            db.get_or_spend(30),
            db.get_or_spend(7),
            db.get_or_spend(1),
        )
