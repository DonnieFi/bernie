...
def build_home_router(ctx: Any) -> APIRouter:
    ...
    async def debug_config(user: _ac.Person = Depends(_ac.require_admin)):
        # family-bot-1a8 residual: family tokens must not enumerate config/members
        return {
            "keys": list(_ac.config.keys()),
            "family_members": list(_ac.config.get("family_members", {}).keys()),
        }

    # --- Presence ---
    @router.get("/api/presence", dependencies=[Depends(_ac.verify_token)])
    async def get_presence():
        # family-bot-julyaudit P1: snapshot-first (align with Today/Family)
        presence, _src = await presence_service.get_full_presence_for_web()
        return presence

    @router.post("/api/presence/webhook")
    async def presence_webhook(
        data: Dict[str, Any],
        x_internal_auth: str | None = Header(default=None, alias="X-Internal-Auth"),
    ):
        # family-bot-1a8: was open no-op; require internal secret (or 410)
        secret = os.environ.get("INTERNAL_POST_SECRET") or ""
        if not secret or x_internal_auth != secret:
            raise HTTPException(status_code=410, detail="presence webhook disabled")
        log.info("Received presence webhook: %s", data)
    ...
    async def set_presence(
        ...
        # family-bot-julyaudit P1: only self or admin may override home state
        target = person_registry.resolve(person_id) or person_id
        actor = person_registry.resolve(user.id) or user.id
        role = (user.role or "").lower()
        if role != "admin" and str(target) != str(actor):
            raise HTTPException(status_code=403, detail="Can only set your own presence")
        await db_writes.update_presence(target, bool(is_home))
        return {"ok": True, "person_id": target, "home": bool(is_home)}
    ...
    async def _get_temps_data(hours: int = 24):
        ...
        if sensor_cfg:
            ...
            sensors = [s for s in await ha_service.get_temperature_sensors()
                       if not any(x in s.get("entity_id", "").lower() for x in exclusions)]
        # family-bot-agc: one HA history batch (not N serial history GETs)
        eids = [s.get("entity_id", "") for s in sensors if s.get("entity_id")]
        hist_map = await ha_service.get_history_batch(eids, hours=hours) if eids else {}
        result = []
        for s in sensors:
            eid = s.get("entity_id", "")
            history = hist_map.get(eid) or []
            name = label_map.get(eid) or s.get("attributes", {}).get("friendly_name", eid)
            temps = [
                float(h.get("state", ""))
                for h in history
                if h.get("state") not in (None, "unavailable", "unknown")
                and str(h.get("state", "")).replace(".", "").replace("-", "").isdigit()
            ]
            result.append({
    ...
    async def get_home_dashboard(hours: int = 24):
        try:
            ...
            # family-bot-co8: surface config-driven favorites with dashboard
            from web_ui_prefs import web_ui_prefs
            _ui = web_ui_prefs(_ac.config if isinstance(_ac.config, dict) else {})
            payload = {
                ...
                "quick_lights": _ui.get("quick_lights") or [],
                "air_quality_rooms": _ui.get("air_quality_rooms"),
            }
