        ...
        try:
            ...
            if is_subscription_enabled(model, config):
                try:
                    ...
                    log.exception(
                        "nightly_digest: subscription chat mining failed for %s",
                        person_name,
                    )
            elif api_key:
                try:
            ...
            # Fallback (must not be a subscription model id)
            if not insights and fallback_model and not is_subscription_enabled(fallback_model, config):
                prompt = build_digest_user_prompt(
                    person_name,
                    formatted,
                    source=f"a chat thread titled '{title}'",
...
async def generate_nightly_digest(config: dict):
    ...
    # Prefer non-Claude active model as fallback, never a subscription id.
    fallback_model = active_model if not active_model.startswith("claude-") else None
    if fallback_model and is_subscription_enabled(fallback_model, config):
        fallback_model = (config.get("llm_fallback") or {}).get("model")
    if fallback_model and is_subscription_enabled(fallback_model, config):
        fallback_model = None
    fallback_base_url = active_base_url if fallback_model else None

    api_key = os.environ.get("ANTHROPIC_API_KEY", "")
    if not api_key and not fallback_model and not is_subscription_enabled(primary_model, config):
    ...
    try:
        async def _one_person(person_name: str, person_id: str) -> None:
            ...
            async with dig_sem:
                try:
                    insights: list[dict] = []
                    subscription_primary = is_subscription_enabled(primary_model, config)
                    if subscription_primary:
                        try:
                            from subscription_invoke import complete_text
                            ...
                            log.exception("nightly_digest: subscription digest failed for %s", person_name)

                    # Never call Anthropic with a Codex/Grok model id after subscription path.
                    if (
                        not subscription_primary
                        and api_key
                        and shared_client is not None
                        and not insights
                    ):
                        try:
                            insights = await _generate_insights_for_person(
