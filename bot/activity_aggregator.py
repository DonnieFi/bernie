...
def _usage_provider(model: str) -> str:
    prefix = (model or "").split("/", 1)[0]
    if prefix in ("codex", "grok", "openrouter", "ollama"):
        return "openai" if prefix == "codex" else prefix
    from model_registry import model_source
    source = model_source(model, config)
    return "openai" if source == "codex" else source
...
async def get_provider_accounts(use_cache: bool = False):
    ...
    active = config.get("active_model", "")
    if active and _usage_provider(active) == "openrouter":
        or_model = active
    else:
        or_model = "—"
...
async def get_activity_dashboard(period: str = "30d", force_refresh: bool = False):
    ...
    for r in current_rows:
        daily_grouped[r["date"]][r["model"]] = {
            "provider": _usage_provider(r["model"]),
            "inTok": r["in_tok"],
            "outTok": r["out_tok"],
    ...
    for r in current_rows:
        provider = _usage_provider(r["model"])
        if provider not in latest_by_provider or r["date"] > latest_by_provider[provider]["date"]:
            latest_by_provider[provider] = r
    accounts_by_provider = {a["provider"]: a for a in accounts}
    for provider, row in latest_by_provider.items():
        account = accounts_by_provider.get(provider)
        if account:
            account["activeModel"] = row["model"]
            account["lastUsedAt"] = row["date"]
            continue
        accounts.append({
            "provider": provider,
            ...
            "lastToppedUp": None,
            "lastUsedAt": row["date"],
            "auth": "oauth" if provider in ("openai", "grok") else "local",
        })
...
async def generate_and_cache_session_title(session_id: str, channel_id: int, start_time: float):
    ...
    try:
        ...
        if is_subscription_enabled(model, config):
            ...
            title_raw = await complete_text(
                model,
                config=config,
                prompt=prompt,
                system=_TITLE_SYSTEM,
                surface="activity_title",
                timeout_s=30,
            )
        else:
        ...
        clean_title = (title_raw or "").strip().strip('"').strip("'").strip()
        if clean_title:
