...
async def handle_litellm_switch_model(args: dict, ctx) -> str:
    ...
    catalog = {e.model: e for e in build_catalog(ctx.config or {})}
    valid_models = set([DEFAULT_MODEL]) | set(catalog.keys())
    if ctx.config:
        if "default_model" in ctx.config:
            valid_models.add(ctx.config["default_model"])

    if name not in valid_models:
        sample = ", ".join(sorted(valid_models)[:12]) or "None"
        return (
            f"❌ Error: Model `{name}` is not registered.\n"
            f"Known models (sample): {sample}"
        )

    from model_registry import model_base_url, model_source
    source = model_source(name, ctx.config or {})
    base_url = None if source in ("anthropic", "codex", "grok") else model_base_url(name, ctx.config or {})
    set_model(name, base_url)
    from config import update_config
    await update_config({"active_model": name})
    from llm.clients import model_cache_support
    entry = catalog.get(name)
    chain = format_chain(entry, name, ctx.config or {})
    return (
        f"Switched to `{name}` (provider `{source}`). "
        f"Chain: `{chain}`. {model_cache_support(name)}"
    )
...
@tool(
    ...
    description=(
        "Secret-safe subscription/provider status: selected model, provider readiness "
        "(codex/grok/anthropic/openrouter/ollama), and usage when the CLI reports it. "
        "Never returns tokens, device codes, or account identity."
    ),
    is_write=False,
    input_schema={
        "type": "object",
        "properties": {
            "provider": {
                "type": "string",
                "description": "Optional filter: codex, grok, or omit for all",
            },
        },
        "required": [],
    },
...
async def handle_get_provider_status(args: dict, ctx) -> str:
    ...
    readiness = provider_readiness_map(cfg, runner_health=health)
    current, _ = get_model_info()
    entry = next((e for e in build_catalog(cfg, runner_health=health) if e.model == current), None)
    filt = (args.get("provider") or "").strip().lower()
    lines = [
        f"Selected model: `{current}`",
        f"Provider: `{entry.provider if entry else 'unknown'}`",
        f"Chain: `{format_chain(entry, current, cfg)}`",
        "Readiness (no secrets):",
    ]
    for key in ("anthropic", "openrouter", "litellm", "ollama", "codex", "grok"):
        if filt and key != filt:
            continue
        lines.append(f"• {key}: {readiness.get(key, 'unavailable')}")
    usage = await subscription_usage_snapshot(cfg, provider=filt or None)
...
@tool(
    ...
    input_schema={"type": "object", "properties": {}, "required": []},
    role_required=ROLE_ALL,
    # family-bot-f88: discovery tool — was mis-tagged notify (rode family notify surface)
    domain="search",
    tier=1,
)
