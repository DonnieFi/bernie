...
async def handle_get_homework(args: dict, ctx) -> str:
    ...
    if timeframe == "week":
        ...
        all_day = homework_due_events(events, ctx.config, monday, monday + timedelta(days=6))
        by_day: dict = {}
        for e in all_day:
            d = e.get("due_date", e["end"] - timedelta(days=1)).strftime("%A, %b %d")
    ...
    due = homework_due_events(events, ctx.config, target)
    if not due:
