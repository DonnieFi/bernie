...
async def _call_shadow_model(
    ...
    """Text-only shadow call. Subscription models use complete_subscription_chain."""
    cfg = config or {}
    try:
        from model_catalog import is_subscription_enabled
        from subscription_invoke import complete_text
        if is_subscription_enabled(model, cfg):
            try:
                ...
                log.exception("shadow subscription path failed model=%s", model)
                # Never fall through to Anthropic with a Codex/Grok model id.
                return None, None, None
    except Exception:
        log.exception("shadow subscription gate failed model=%s", model)
        try:
            from model_catalog import is_subscription_enabled
            if is_subscription_enabled(model, cfg):
                return None, None, None
        except Exception:
            # Unknown gate failure — refuse Anthropic with possible sub model id.
            from model_registry import model_source
            if model_source(model, cfg) in ("codex", "grok"):
                return None, None, None
