...
async def _run_judge_with_fallbacks(prompt: str, result_type, primary_model: str, label: str):
    ...
    Subscription (Codex/Grok) tiers use complete_subscription_chain + parse;
    never ServiceContainer.llm_for or Pydantic factories that reject them.
    """
    ...
    for idx, model in enumerate(chain):
        try:
            ...
            if is_subscription_enabled(model, _cfg):
                ...
                if parsed is None:
                    # ConnectionError is treated as transient → next fallback tier.
                    raise ConnectionError(
                        f"{label}: subscription judge returned empty model={model}"
                    )
                # Match pydantic_ai AgentRunResult shape used by callers.
                result = SimpleNamespace(output=parsed, usage=None)
            else:
