...
def _alias_table() -> dict[str, str]:
    ...
    # Common Bernie aliases not yet in the JSON table (non-subscription models).
    extras = {
    ...
    try:
        ...
        from model_slug_map import openrouter_alias_table

        for alias, slug in openrouter_alias_table(live_config).items():
            aliases.setdefault(alias, slug)
            aliases.setdefault(alias.lower(), slug)
    except Exception:
        log.debug("openrouter_models: subscription alias merge skipped", exc_info=True)
...
def register_openrouter_alias(alias: str, openrouter_slug: str) -> None:
    ...
    try:
        from model_slug_map import invalidate_slug_cache
...
def resolve_openrouter_slug(model: str | None, cfg: dict | None = None) -> str:
    """Map Bernie/Codex/LiteLLM id to OpenRouter slug (openai/gpt-5.4-mini)."""
    if not model:
        return model or ""
    if "/" in model:
        return model
    ...
    try:
        from model_slug_map import resolve_openrouter

        mapped = resolve_openrouter(model, live_cfg)
        if mapped:
            return mapped
    except Exception:
        log.debug("resolve_openrouter_slug: subscription map lookup failed", exc_info=True)
