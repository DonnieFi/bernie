...
class _AnthropicSmolModel:
    ...
    def generate(self, messages, stop_sequences=None, response_format=None, tools_to_call_from=None, **kwargs):
        ...
        # family-bot-21n: Bernie system blocks (with cache_control) first; agent
        # scaffolding appended as plain text — matches native left-to-right cache.
        bernie_blocks = getattr(self, "_bernie_system_blocks", None) or []
        if bernie_blocks or system_parts:
            system_payload: list | str
            if bernie_blocks:
                system_payload = [dict(b) for b in bernie_blocks if isinstance(b, dict)]
                if system_parts:
...
class SmolExecutor:
            ...
            def _make_sync_fn(tool_name, exec_config):
                def sync_fn(**kwargs):
                    ...
                    try:
                        ...
                        # family-bot-otc.2: never leak stackish copy into agent/user text
                        log.exception("SmolExecutor tool %r raised", tool_name)
                        return (
                            f"Tool {tool_name} could not finish just then. "
    ...
    async def _run_core(
        ...
        # family-bot-21n: do NOT flatten Bernie system into the user task —
        # that kills prompt-cache prefixes. Keep blocks (with cache_control)
        # on the model; agent.run gets the user task only.
        if isinstance(system, list):
            smol_model._bernie_system_blocks = [
                dict(b) for b in system if isinstance(b, dict) and b.get("text")
            ]
            log_system = "\n\n".join(b.get("text", "") for b in smol_model._bernie_system_blocks)
        elif system:
            smol_model._bernie_system_blocks = [{"type": "text", "text": str(system)}]
            log_system = str(system)
        else:
            smol_model._bernie_system_blocks = []
            log_system = ""
        ...
        try:
            ...
            await _log_smol_generation(
                config=config,
                services=self._services,
                user_input=(log_system[:200] + " | " + task) if log_system else task,
                output=result_text,
                input_tokens=tok_in,
            ...
            # family-bot-otc.2: full exception in logs only — family Discord gets calm copy
            log.exception("SmolExecutor: CodeAgent failed: %s", exc)
            err_text = (
                "I hit a snag finishing that — try asking again in a moment."
            )
            tok_in = getattr(smol_model, "tokens_in_total", 0) or 0
            tok_out = getattr(smol_model, "tokens_out_total", 0) or 0
            await _log_smol_generation(
