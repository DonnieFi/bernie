...
def _is_prompt_cache_error(err_msg: str) -> bool:
    """True only for prompt-cache / cache_control provider rejections.

    family-bot-8z7: bare \"not supported\" is too broad (tool/model errors) and
    used to trigger a silent max_tokens=1024 retry.
    """
    msg = (err_msg or "").lower()
    if "cache_control" in msg or "prompt caching" in msg or "prompt cache" in msg:
        return True
    # e.g. "caching is not supported by this model"
    if "cach" in msg and "not supported" in msg:
        return True
    return False
...
class NativeToolExecutor:
    ...
    async def run(
        ...
        try:
        ...
        except MissingProviderClient as exc:
            log.warning("NativeToolExecutor provider unavailable model=%s: %s", config.model, exc)
            fb = (app_config.get("llm_fallback") or {}).get("model")
            ollama_models = app_config.get("ollama_models") or []
            if fb and fb in ollama_models:
                from ollama_resolver import configured_ollama_base_url

                if configured_ollama_base_url(app_config):
            ...
            return "I couldn't reach the model provider just now — try again in a moment."
        ...
        for _ in range(_max_steps):
            ...
            try:
                ...
                # family-bot-8z7: do NOT match bare "not supported" (truncates + extra RTT).
                err_msg = str(e).lower()
                is_cache_error = _is_prompt_cache_error(err_msg)
                ...
                if is_cache_error:
                    ...
                    # Persist stripped payloads so later tool-loop steps do not
                    # re-hit the same cache rejection each round.
                    system = fallback_system
                    tools = fallback_tools

                    # Retry once without caching — keep original max_tokens
                    response = await _create_messages(
                        model=config.model,
                        max_tokens=_max_tokens,
                        system=system,
            ...
            # family-bot-px2: run independent tool_use blocks concurrently
            ...
            async def _run_one(block):
                ...
                try:
                    ...
                    log.exception("NativeToolExecutor tool %r raised", block.name)
                    # family-bot-otc.2: no stackish tool text to the model/user path
                    result = (
                        f"Tool {block.name} could not finish just then. "
            ...
            if tool_blocks:
                tool_results = list(await asyncio.gather(
                    *(_run_one(b) for b in tool_blocks),
                ))
                for block in tool_blocks:
                    if _validation_failures.get(block.name, 0) >= 2:
                        reliable = (
                        ...
                        if config.model != reliable:
                            log.warning(
                                "Escalating from %s to %s after validation failures",
                                config.model, reliable,
                            )
                            from model_catalog import is_subscription_enabled
                            if is_subscription_enabled(reliable, app_config):
                                # Subscription models cannot use Anthropic clients;
                                # synthesize a final answer via complete_subscription_chain.
                                try:
                                    from subscription_invoke import complete_text

                                    def _system_text(blocks) -> str | None:
                                    ...
                                    note = (
                                        "Tool validation failed repeatedly. Answer the user "
                                        "using what you already know; do not invent tool results."
                                    )
                                    user_bits = []
                                    for m in messages:
                                        if m.get("role") == "user" and isinstance(
                                            ...
                                            user_bits.append(m["content"])
                                    synth = await complete_text(
                                        reliable,
                                        config=app_config,
                                        prompt="\n".join(user_bits[-3:] + [note]) or note,
                                        system=_system_text(system),
                                        surface="primary_reliable",
                                    )
                                    if synth:
                                        return _finish(synth)
                                except Exception:
                                    log.exception(
                                        "primary_reliable subscription escalation failed model=%s",
                                        reliable,
                                    )
                                # Leave current model/client if subscription fails.
                            else:
                                config = ExecutorConfig(
                                    **{**config.__dict__, "model": reliable}
                                )
