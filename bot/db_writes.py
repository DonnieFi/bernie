...
async def log_activity(
    ...
    # family-bot-29q: activity log must not block chat/presence on RPC storms
    await cognition_db_write(
        "log_activity",
        required=False,
        rpc_class="batch",
...
async def apply_presence_tick(updates: list[dict]) -> list[tuple[str, bool]]:
    ...
    return await cognition_db_write(
        "apply_presence_tick", rpc_class="ux", updates=updates
    )
...
async def routed(op: str, /, *args: Any, **kwargs: Any) -> Any:
    ...
    if args:
        ...
        bound.update(kwargs)
        kwargs = bound
    # family-bot-29q: telemetry ops never block callers via routed()
    if op in ("log_activity", "log_token_usage"):
