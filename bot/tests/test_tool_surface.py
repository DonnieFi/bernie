...
class TestModeCeiling(unittest.TestCase):
    ...
    def test_ops_full_surface(self):
        ...
        self.assertIn("kanban", ceiling)
        self.assertIn("modes", ceiling)
        from tool_gateway import ToolGateway
        names = {
            schema["name"]
            for schema in get_tool_schemas_for_turn(
                ToolGateway(registry=get_registry()), "admin", ceiling, {},
            )
        }
        self.assertIn("kanban_show", names)
...
class TestGetToolSchemasForTurn(unittest.TestCase):
    def test_filters_by_resolved_domains(self):
        ...
        load_all_domains()
        gw = ToolGateway(registry=get_registry())
        # family-bot-f88: list_slash is domain=search (discovery), not notify
        schemas = get_tool_schemas_for_turn(gw, "family", ["search"], {})
        names = [s["name"] for s in schemas]
