...
class TestWorkerRetryEscalatesToUpgradeModel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retry_uses_upgrade_model_when_configured(self):
        ...
        async def fake_topic(model, topic, config, num_ctx=None, system=None, timeout_s=300):
            ...
            return (good_json if len(call_models) == 3 else bad_json, stats)
        ...
        self.assertEqual(call_models, [
            ...
            "hermes3:8b-llama3.1-q6_K",
            "qwen2.5:14b",
        ], "validation-feedback retry must escalate to upgrade_model")
...
class TestEmptyOllamaTextRaises(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reflection_raises_on_empty_then_empty(self):
        ...
        # Initial, empty retry, and validation-feedback retry all stay empty.
        with patch("worker._call_ollama_topic",
                   new=AsyncMock(side_effect=[("", stats), ("", stats), ("", stats)])):
            with self.assertRaises(RuntimeError) as ctx:
                await ReflectionWorker(cfg).handle(task, container=_worker_container(db))

        self.assertIn("model returned no text", str(ctx.exception))
    ...
    def test_scheduled_consolidation_does_not_repeat_internal_retries(self):
        from pathlib import Path

        source = (Path(__file__).resolve().parents[1] / "bot.py").read_text()
        enqueue = source.split("async def consolidation_enqueue_task", 1)[1].split(
            "async def routine_decay_task", 1
        )[0]
        self.assertIn("max_retries=1", enqueue)
    ...
    async def test_research_query_empty_does_not_raise(self):
        ...
        with patch("worker._call_ollama_topic", side_effect=fake_ollama), \
             patch("cognitive_workers.research.get_http_session", return_value=MagicMock()):
        ...
        # Initial, empty retry, and validation-feedback retry all return no
        # queries; the iteration then ends naturally and synth runs once.
        self.assertEqual(call_log.count("query"), 3)
