...
class TestSyncFnErrorPaths(unittest.TestCase):
    ...
    def test_sync_fn_returns_error_string_on_exception(self):
        ...
        self.assertIn("Tool get_todays_events could not finish", result)
...
class TestIntentRouting(unittest.TestCase):
    ...
    def test_looks_multistep_simple_lookup_is_false(self):
        ...
        # family-bot-9xg: bare plan/each/?? must not force smol
        self.assertFalse(_looks_multistep("what's the plan?", {}))
        self.assertFalse(_looks_multistep("can you reach her?", {}))  # no bare each
        self.assertFalse(_looks_multistep("huh??", {}))
