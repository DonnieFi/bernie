...
class TestEnsureStudyTask(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_enqueue_then_skip(self):
        ...
        async def create_locally(op, **kwargs):
            self.assertEqual(op, "create_cognitive_task")
            return await db.create_cognitive_task(**kwargs)

        with patch("db_writes.routed", side_effect=create_locally):
    ...
    async def test_enqueue_routes_write_to_cognition(self):
        from cognitive_workers.study_detection import ensure_study_task

        with patch("db_writes.routed", new_callable=AsyncMock, return_value=42) as routed:
            task_id = await ensure_study_task(
                {"id": "ev_route", "summary": "Math Exam"}, person_id="child1"
            )

        self.assertEqual(task_id, 42)
        self.assertEqual(routed.await_args.args, ("create_cognitive_task",))
        self.assertEqual(routed.await_args.kwargs["type"], "study_guide")


@unittest.skipUnless(db is not None, "database not available")
class TestRequeueCognitiveTask(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self._tmpdir = tempfile.TemporaryDirectory()
        self._old_db_path = db.DB_PATH
        db.DB_PATH = os.path.join(self._tmpdir.name, "requeue.db")
        await db.init_db()
    ...
    async def test_requeue_dead_letter_resets_task(self):
        tid = await db.create_cognitive_task(
            type="consolidation",
            payload={"person_id": "child1"},
        )
        await db.fail_cognitive_task(tid, "boom")
        await db.fail_cognitive_task(tid, "boom")
        await db.fail_cognitive_task(tid, "boom")
        row = await db.get_cognitive_task(tid)
        self.assertEqual(row["status"], "dead_letter")

        ok = await db.requeue_cognitive_task(tid)
        self.assertTrue(ok)
        row = await db.get_cognitive_task(tid)
