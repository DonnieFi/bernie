...
class TestPhase24Infrastructure(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_watchman_remote_health(self, mock_get_state):
        ...
        def side_effect(eid):
            if eid == "binary_sensor.pihole_primary_status":
                return {"state": "on"}
            if eid == "binary_sensor.pihole_secondary_status":
                return {"state": "off"}
            return None
        ...
        entities = ["binary_sensor.pihole_primary_status", "binary_sensor.pihole_secondary_status"]
        with patch("watchman.config", {"watchman": {"health_entities": entities}}):
            wm = Watchman()
            results = await wm.get_remote_health()
        
        self.assertEqual(results["binary_sensor.pihole_primary_status"], "Healthy")
        self.assertEqual(results["binary_sensor.pihole_secondary_status"], "Offline")
