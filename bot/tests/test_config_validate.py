...
class TestConfigValidate(unittest.TestCase):
    ...
    def test_valid_subscription_models(self):
        cfg = self._minimal()
        cfg["subscription_models"] = [{
            "provider": "grok",
            "model": "grok-4.5",
    ...
    def test_subscription_empty_openrouter_mapping_raises(self):
        cfg = self._minimal()
        cfg["subscription_models"] = [{
            "provider": "codex",
            "model": "gpt-5.4",
    ...
    def test_subscription_invalid_provider_raises(self):
        cfg = self._minimal()
        cfg["subscription_models"] = [{
            "provider": "anthropic",
            "model": "claude",
            "capabilities": ["text"],
            "openrouter_fallback_model": "x",
            "enabled": True,
        }]
        with self.assertRaises(ValueError):
            validate_config_core(cfg)
