...
class TestCacheBusterAndPanelWired(unittest.TestCase):
    def test_nav_ia_six_items(self):
        ...
        self.assertIn('label: "Family"', app_js)
        self.assertIn('label: "Door"', app_js)
    ...
    def test_cache_buster_v100(self):
        idx = _web("index.html").read_text()
        self.assertIn("app.v6.js?v=100", idx)
        self.assertNotIn("?v=25", idx)
        app = (_bot("api") / "app.py").read_text()
        self.assertIn('request.url.path == "/"', app)
