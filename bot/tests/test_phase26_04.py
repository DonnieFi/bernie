...
class TestResearchWorkerQueryRetry(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_query_retry_recovers_then_continues(self):
        ...
        self.assertTrue(any("no usable output" in m for m in logs.output))
    ...
    async def test_query_retry_both_fail_stops_iteration(self):
        ...
        # Three query calls (initial, same-model retry, validation-feedback
        # retry), then iteration ends and synth runs on the empty findings set.
