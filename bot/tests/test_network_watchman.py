...
class NetworkWatchmanHelpersTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_parse_caddy_ips(self):
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".Caddyfile", delete=False) as f:
            ...
            path = f.name
        try:
            m = await _parse_caddy_ips(path)
            self.assertIn("ha.lan", m)
            self.assertIn("192.168.1.X", m["ha.lan"])
        finally:
            os.unlink(path)

    async def test_parse_caddy_ips_timeout_returns_empty(self):
        from network_watchman import _parse_caddy_ips
        async def _timeout(*args, **kwargs):
            raise TimeoutError

        with patch("network_watchman.asyncio.to_thread", side_effect=_timeout):
            self.assertEqual(await _parse_caddy_ips("/unavailable/Caddyfile"), {})
    ...
    async def test_parse_caddy_ips_block_style(self):
        from network_watchman import _parse_caddy_ips
        content = (
            "# legacy inline proxy\n"
            "ha.lan {\n"
    ...
    async def test_caddy_check_runs_when_due(self):
        ...
        with (
            ...
            patch("network_watchman._parse_caddy_ips", new_callable=AsyncMock, return_value={"grafana.lan": {"192.168.1.X"}}) as mock_parse,
            patch.dict(sys.modules, {"network_service": SimpleNamespace(network_service=fake_network)}),
        ):
            events = await poll()
