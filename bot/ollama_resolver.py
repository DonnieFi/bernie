...
def is_placeholder_ollama_url(url: str | None) -> bool:
    """True when URL is unset or still the documented placeholder."""
    if not url or not isinstance(url, str):
        return True
    u = url.rstrip("/")
    return _PLACEHOLDER_HOST in u or u == _DEFAULT.rstrip("/")


def configured_ollama_base_url(config: dict) -> str | None:
    """Primary Ollama base URL, or None when only the placeholder is configured."""
    url = current_ollama_base_url(config)
    return None if is_placeholder_ollama_url(url) else url
