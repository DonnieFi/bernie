...
class ServiceContainer:
    ...
    def llm_for(self, model: str) -> Any:
        ...
        if source == "ollama":
            if not self.ollama:
                raise MissingProviderClient("ollama", model)
            return self.ollama  # str — caller branches to _call_ollama
        if source == "openrouter":
            client = self.openrouter or self.litellm
            if client is None:
                raise MissingProviderClient("openrouter", model)
            return client
        if source in ("codex", "grok"):
            raise ValueError(
                f"subscription model {model!r} must route through CompletionRouter, not llm_for"
            )
        if self.litellm is None:
            raise MissingProviderClient("litellm", model)
        return self.litellm
