...
ensure_venv() {
    ...
    if [[ ! -f "$lock_stamp" ]] || [[ bot/requirements.lock -nt "$lock_stamp" ]] ||
       ! "$PY" -c "import pydantic" 2>/dev/null; then
...
# Leg 3: subscription_runner sidecar (PYTHONPATH=repo root)
mapfile -t RUNNER_TESTS < <(_load_gate_list "RUNNER_TESTS")

run_host_bot_tests() {
    echo -e "\n=== host: bot/tests (curated named modules) ==="
    for module in "${BOT_TESTS[@]}"; do
        PYTHONPATH=bot "$PY" -m unittest -v "$module"
    done
}
...
run_runner_tests() {
    if [[ ${#RUNNER_TESTS[@]} -eq 0 || -z "${RUNNER_TESTS[0]:-}" ]]; then
        echo -e "\n=== host: subscription_runner (RUNNER_TESTS empty — skipped) ==="
        return 0
    fi
    echo -e "\n=== host: subscription_runner (curated named modules) ==="
    for module in "${RUNNER_TESTS[@]}"; do
        PYTHONPATH=. "$PY" -m unittest -v "$module"
    done
}
