...
A self-hosted Discord bot for household coordination — calendar, presence, smart home, meals, tasks, cameras, and optional email. Chat uses **your** configured LLM (Claude, GPT, OpenRouter, Ollama, …) plus tools. Background jobs often run on local Ollama.
...
Bernie is provider-agnostic. Common paths:

- **Anthropic** direct (`claude-*` + `ANTHROPIC_API_KEY`)
- **OpenRouter** direct (`or-*` + `OPENROUTER_API_KEY`, optional low-cost Luna)
- **LiteLLM / OpenAI-compatible proxy** (`litellm_base_url` + proxy models)
- **Local Ollama** for chat fallback and overnight workers
- **Optional ChatGPT/Codex OAuth** via the isolated subscription runner — see [OpenAI models and Codex OAuth](../subscription-providers.md)

Grok OAuth is not a supported public setup yet. Switch live with `/model` in `#anvil`. See `active_model`, `openrouter_direct`, `litellm_base_url`, `ollama_base_url`, and `subscription_models` in config.
...
Depends on the vendor and tier. Check that provider’s current API/data policy (Anthropic, OpenAI, OpenRouter, …). Local Ollama stays on your hardware.
...
`config.json` is gitignored on purpose so live household secrets never land in git. If a pull or clean checkout leaves you without a working file:

1. Restore the latest **off-git** backup (`config.json.bak-YYYYMMDD`, secrets store, host snapshot).
2. If you have no backup, start from the template and re-enter IDs/tokens:
   ```bash
   cp config.minimal.example.json config.json   # or config.example.json for the full shape
   $EDITOR config.json
   ```

Never expect a public clone to contain a real household `config.json`. Always keep dated backups **outside git**. See [Deploy guide](../deploy.md).
