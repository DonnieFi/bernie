...
| LLM credentials | Anthropic API key, OpenRouter API key for Luna, or optional Codex OAuth |
...
| `ANTHROPIC_API_KEY` or `OPENROUTER_API_KEY` | One route required* | Anthropic and/or OpenRouter (*or Ollama / Codex OAuth — see quickstart) |
| `SUBSCRIPTION_RUNNER_SECRET` | Codex OAuth only | Shared secret for the isolated runner |
...
`config.json` is intentionally not in git (household IDs, HA tokens, entity maps). A pull or clean clone will **not** bring a working config with it.

**Recovery**

1. Prefer an off-git backup: `config.json.bak-YYYYMMDD`, secrets store, or host snapshot.
2. Fresh install only: copy a template and fill values:
   ```bash
   cp config.minimal.example.json config.json   # hour-one
   # or: cp config.example.json config.json     # full optional blocks
   ```

**Do not rely on the example alone** for an existing production deployment — templates omit family-specific blocks (HA entity maps, calendars, `network_watchman.critical_hosts`, Frigate camera labels, etc.). Always keep dated backups **outside git** before pulling or editing.
...
The `subscriptions` Compose profile adds an internal-only, non-root sidecar
containing the official Codex CLI. It has no source, database, general
credentials, documentation, host-home, or Docker-socket mounts. OAuth state
is held only in its writable named volume; never copy it into `credentials/` or
mount host `~/.codex`.

Codex requests run through one ephemeral `codex app-server` thread. Native
dynamic-tool requests pause at the runner boundary, execute through Bernie's
ToolGateway, then resume the same turn. The app-server is supervised inside the
existing runner container; do not deploy a second app-server service.

Generate `SUBSCRIPTION_RUNNER_SECRET` in `.env`, start the profile, and use
the official device login:

```bash
openssl rand -hex 32  # paste into .env as SUBSCRIPTION_RUNNER_SECRET
docker compose --profile subscriptions up -d --build bernie-subscription-runner
docker compose --profile subscriptions run --rm --entrypoint codex \
  bernie-subscription-runner login --device-auth
docker compose --profile subscriptions run --rm --entrypoint codex \
  bernie-subscription-runner login status
```

Reauthenticate when health reports `reauth-required`. To stop using OAuth,
disable the Codex model in `subscription_models` and select a normal API or
local model. The volume intentionally preserves the OAuth session until you
choose to remove it.

Defaults are two active CLI requests and four queued; tune with
`SUBSCRIPTION_RUNNER_MAX_ACTIVE`, `SUBSCRIPTION_RUNNER_MAX_QUEUE`, and
`SUBSCRIPTION_RUNNER_QUEUE_TIMEOUT_S`. Capacity overflow returns a typed busy
response so Bernie can continue through its configured OpenRouter or Ollama
route. Grok OAuth is still in progress and is not part of this deployment path.
...
```bash
PYTHONPATH=bot python scripts/measure_tool_surface.py
```
