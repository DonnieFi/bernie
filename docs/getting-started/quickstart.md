...
| One LLM path | Anthropic, OpenRouter, local Ollama, or optional ChatGPT/Codex OAuth — pick any one to start |
| Python 3.11+ (host) | Only for one-time Google OAuth scripts — optional at first |

Bernie is **provider-agnostic**. Hour one only needs *some* model route that can answer in Discord.
...
# Pick at least one LLM path (examples — not all required):
# ANTHROPIC_API_KEY=…                 # Claude models (active_model: claude-…)
# OPENROUTER_API_KEY=…                # OpenRouter models (active_model: or-…)
# # local Ollama: set ollama_base_url + ollama_models in config.json
# # ChatGPT/Codex OAuth: see docs/subscription-providers.md (optional)
```

Match `active_model` / `webui_model` in `config.json` to whatever path you enabled. Full variable list: [Environment reference](../reference/environment.md). Optional OpenRouter Luna / Codex OAuth: [OpenAI models and Codex OAuth](../subscription-providers.md).
...
Required keys for hour-one: `guild_id`, `schedule_channel_id`, `anvil_channel_id`, `admin_discord_id`, `family_members`, `timezone`, and an `active_model` that matches your LLM credentials.
...
| Optional ChatGPT/Codex OAuth | [OpenAI models and Codex OAuth](../subscription-providers.md) |
