...
(function () {
  ...
  function renderHeader() {
    ...
    // family-bot-q69: Door modes in Bernie voice (API still on/off/test)
    const modes = [
      { id: "on", label: "Watching", help: "Alerts when someone should notice." },
      { id: "off", label: "Quiet", help: "Cameras stay up; Bernie stays quiet." },
      { id: "test", label: "Practice", help: "Safe dry-run — no family pings." },
    ];
    const activeHelp = (modes.find(m => m.id === mode) || modes[0]).help;

    const selector = el("div", { class: "mode-selector", role: "group", "aria-label": "Door mode" },
    ...
    header.append(
      el("div", { class: "col", style: "gap: 6px; flex: 1;" },
        el("h2", {}, "Door"),
        el("div", {
          class: "t-body",
          style: "font-size: 12.5px; color: var(--ink-3); font-style: italic; max-width: 36ch;",
        }, activeHelp)
      ),
      selector
    );
  ...
  async function refreshCameraImage(camId, imgEl) {
    ...
    // family-bot-u9m: drop late blobs if user left Security during fetch
  ...
  async function renderEvents() {
    ...
    if (state.events.length === 0) {
      list.append(el("div", { style: "color: var(--ink-2); font-style: italic;" }, "All quiet. That's good."));
      return;
    }
  ...
  window.renderCameras = async function () {
    ...
    await loadData();
    // family-bot-u9m: user may have left during await — do not re-arm poll or paint
    if (!isSecurityPanelActive()) {
      if (typeof window.v3CamerasLeave === "function") window.v3CamerasLeave();
      else {
  ...
  // family-bot-u9m: leave hook (like v3AdminLeave) — do not monkey-patch showPanel;
  // app.v6.js reassigns window.showPanel and would overwrite any wrap.
  window.v3CamerasLeave = function () {
    if (state.refreshInterval) {
      clearInterval(state.refreshInterval);
      state.refreshInterval = null;
    }
