...
(function() {
  ...
  async function refreshModelsCatalog() {
    if (!window.api) return null;
    try {
      const fresh = await window.api("/api/config/models");
      if (fresh) {
        window.BernieData = window.BernieData || {};
        window.BernieData.models = fresh;
      }
      return fresh;
    } catch (e) {
      console.error("Failed to refresh model catalog:", e);
      return null;
    }
  }
  ...
  async function renderSettings() {
    ...
    function _renderModelsInto(wrap) {
      ...
      // Provider-aware pools from API (target-filtered catalog). Fallback to
      // catalog/models when pools missing (older API).
      const poolFor = (target) => {
        if (models.pools && models.pools[target]) return models.pools[target];
        if (target === "fallback" || target === "vision" || target === "judge_ollama") {
          return models.ollama_models || [];
        }
        if (target === "judge_fallback") {
          return (models.models || []).filter(m => m.source === "litellm" || m.provider === "litellm" || m.provider === "openrouter");
        }
        if (target === "active" || target === "discord") {
          return (models.pools && models.pools.active) || models.models || [];
        }
        return (models.pools && models.pools[target]) || models.catalog || models.models || [];
      };

      const authLabel = (provider) => {
        if (provider === "grok" || provider === "codex") return "OAuth";
      ...
      const readinessColor = (r) => {
        if (r === "ready") return "var(--ok)";
        if (r === "reauth-required") return "var(--warn)";
        if (r === "unavailable" || r === "disabled") return "var(--err)";
        return "var(--ink-4)";
      };

      const formatModelOption = (m) => {
        if (m.display) return m.display;
        const prov = m.provider || m.source || "";
        const bits = [m.id, authLabel(prov)];
        if (prov) bits.push(prov);
        if (m.readiness && m.readiness !== "ready") bits.push(m.readiness);
        return bits.join(" · ");
      };

      const sortPool = (pool) => {
        const rank = { grok: 0, codex: 0, anthropic: 1, openrouter: 2, litellm: 2, ollama: 3 };
        return [...(pool || [])].sort((a, b) => {
          const ra = rank[a.provider] ?? 2;
          const rb = rank[b.provider] ?? 2;
      ...
      const catalogById = () => {
        const rows = models.catalog || models.models || [];
        const map = {};
      ...
      const formatChain = (chain) => {
        ...
        return chain.map(h => `${h.provider}/${h.model}`).join(" → ");
      };
      ...
      const targets = [
        { key: "discord",   label: "Discord",             pool: poolFor("active") },
        { key: "webui",     label: "Web UI / Ask Bernie", pool: poolFor("webui") },
        { key: "openwebui", label: "OpenWebUI",           pool: poolFor("openwebui") },
        { key: "digest",    label: "Nightly digest",      pool: poolFor("digest") },
        { key: "fallback",  label: "Ollama fallback",     pool: poolFor("fallback") },
        { key: "shadow",    label: "Shadow eval",         pool: poolFor("shadow") },
        { key: "worker",    label: "Background worker",   pool: poolFor("worker") },
        { key: "research",  label: "Research worker",     pool: poolFor("research") },
        { key: "research_upgrade", label: "Research upgrade (escalation)", pool: poolFor("research_upgrade") },
        { key: "study_guide", label: "Study guide worker", pool: poolFor("study_guide") },
        { key: "audit",    label: "Watchman audit",       pool: poolFor("audit") },
        { key: "eval",     label: "Nightly judge (eval)", pool: poolFor("eval") },
        { key: "judge_fallback", label: "Judge LiteLLM fallback", pool: poolFor("judge_fallback") },
        { key: "judge_ollama", label: "Judge Ollama fallback", pool: poolFor("judge_ollama") },
        { key: "vision",   label: "Vision",               pool: poolFor("vision") },
        { key: "primary_reliable", label: "Primary reliable (native escalation)", pool: poolFor("primary_reliable") },
        { key: "reflection", label: "Reflection worker",  pool: poolFor("reflection") },
        { key: "consolidation", label: "Consolidation worker", pool: poolFor("consolidation") },
      ];

      const subs = models.subscription_providers || {};
      const byId = catalogById();
      const discordEntry = byId[models.current];
      const discordChain = formatChain(
        models.current_fallback_chain || (discordEntry && discordEntry.fallback_chain)
      );

      wrap.append(
        el("div", { class: "t-h2", style: "margin-bottom: 4px" }, "Model"),
        el("div", { class: "t-meta", style: "color: var(--ink-4); margin-bottom: 12px" },
          "Subscription models (Grok/Codex) use OAuth via the runner sidecar; others use API keys or local Ollama."
        ),
        el("div", {
          style: "margin-bottom: 16px; padding: 12px; background: var(--bg-sub); border-radius: var(--r-md); border: 1px solid var(--stroke)"
        },
          el("div", { class: "t-eyebrow", style: "margin-bottom: 8px" }, "Subscription OAuth (runner)"),
          el("div", { class: "row gap-3", style: "flex-wrap: wrap" },
            ["grok", "codex"].map(p => {
              const st = subs[p] || "unknown";
              return el("div", {
                class: "row gap-2",
                style: "align-items: center; padding: 6px 10px; background: var(--bg-card); border-radius: var(--r-sm); border: 1px solid var(--stroke)"
              },
                el("span", { class: "t-mono", style: "font-size: 12px; text-transform: uppercase" }, p),
                el("span", {
                  class: "chip",
                  style: `font-size: 10px; color: ${readinessColor(st)}; border-color: color-mix(in oklch, ${readinessColor(st)} 35%, transparent)`
                }, st)
              );
            })
          ),
          el("div", { class: "t-meta", style: "margin-top: 10px; font-size: 11px; color: var(--ink-4)" },
            "Re-auth on BernieHost: ",
            el("span", { class: "t-mono" }, "./scripts/subscription_grok_login.sh"),
            " · Discord: ",
            el("span", { class: "t-mono" }, "/model-auth")
          )
        ),
        models.current ? el("div", {
          style: "margin-bottom: 16px; padding: 10px 12px; background: var(--bg-sub); border-radius: var(--r-md); font-size: 12px"
        },
          el("div", { class: "t-eyebrow", style: "margin-bottom: 4px" }, "Discord active"),
          el("div", { class: "t-mono", style: "color: var(--ink-2)" },
            models.current,
            models.current_provider ? ` · ${authLabel(models.current_provider)} (${models.current_provider})` : "",
            models.current_readiness ? ` · ${models.current_readiness}` : ""
          ),
          discordChain ? el("div", { class: "t-meta", style: "margin-top: 6px; font-size: 11px; color: var(--ink-4)" },
            "Fallback chain: ", discordChain
          ) : null
        ) : null,
        ...targets.map(({ key, label, pool }) => {
          ...
          sortPool(pool).forEach(m => {
            const opt = el("option", { value: m.id }, formatModelOption(m));
          ...
          sel.addEventListener("change", async () => {
            ...
            try {
              ...
              if (key === "discord") {
                models.current = model_id;
                const picked = (pool || []).find(m => m.id === model_id);
                if (picked) {
                  models.current_provider = picked.provider;
                  models.current_readiness = picked.readiness;
                  models.current_fallback_chain = picked.fallback_chain || [];
                }
              }
              await refreshModelsCatalog();
              updateModelsSection();
    ...
    function buildFamilySection(members) {
      return el("div", { class: "card card-pad", style: "margin-bottom: 20px" },
        ...
        el("p", { class: "sub", style: "margin: 0 0 16px" }, "Edit members in Family"),
