...
(function () {
  ...
  // family-bot-wze measure: count API calls during cold unlock → first panel paint
  let _unlockTrace = null; // { paths: string[] } | null
  function beginUnlockTrace() {
    _unlockTrace = { paths: [] };
  }
  function endUnlockTrace(label) {
    if (!_unlockTrace) return;
    const paths = _unlockTrace.paths.slice();
    _unlockTrace = null;
    try {
      console.info(
        `[wze] cold unlock API count=${paths.length} (${label || "unlock"}):`,
        paths
      );
      window.__bernieUnlockApiCalls = paths;
    } catch (_) {}
  }
  ...
  const api = (path, opts = {}) => {
    ...
    if (_unlockTrace) {
      try { _unlockTrace.paths.push(`${method} ${String(path).split("?")[0]}`); } catch (_) {}
    }
  ...
  /** family-bot-0ty: kid web lens (Discord roles mirrored on Me.role). */
  function isKidUser() {
    const role = ((window.Me && window.Me.role) || "").toLowerCase();
    return role === "kids" || role === "kid" || role === "child";
  }
  window.isAdminUser = isAdminUser;
  window.isKidUser = isKidUser;

  /** family-bot-wze: lazy Leaflet (not on cold unlock); retry after failed load */
  let _leafletPromise = null;
  function ensureLeaflet() {
    if (window.L) return Promise.resolve(window.L);
    if (_leafletPromise) return _leafletPromise;
    _leafletPromise = new Promise((resolve, reject) => {
      const cssHref = "/static/vendor/leaflet/leaflet.css?v=82";
      ...
      s.src = "/static/vendor/leaflet/leaflet.js?v=82";
      s.async = true;
      s.onload = () => resolve(window.L);
      s.onerror = () => {
        _leafletPromise = null; // allow retry
  ...
  const NAV = [
    ...
    // family-bot-b7w: human labels Family/Door; hash ids people/security stay for aliases
    { id: "people",   label: "Family",   icon: "family"   },
    { id: "security", label: "Door",     icon: "camera"   },
  ...
  function _normPersonId(id) {
    if (!id) return "";
    const s = String(id);
    return s.startsWith("person:") ? s.slice(7) : s;
  }

  function _personMatches(stored, userId) {
    if (!stored || !userId) return false;
    if (stored === userId) return true;
    const a = _normPersonId(stored);
    const b = _normPersonId(userId);
    return !!a && a === b;
  }

  /** Incomplete chores assigned to the signed-in person (family-bot-0c6 / d3t badge). */
  window.countMyOpenChores = function countMyOpenChores() {
    const tasks = D.tasks || [];
    const meId = (window.Me && window.Me.id) || "";
    if (!meId || !Array.isArray(tasks)) return 0;
    let n = 0;
    for (const t of tasks) {
      const type = t.type || "chore";
      if (type !== "chore") continue;
      const status = String(t.kanban_status || t.status || "").toLowerCase();
      if (status === "done" || status === "archived" || status === "complete") continue;
      if (t.completed_at) continue;
      // API "pending" / kanban todo|ready|running|blocked all count as open
      if (status === "pending" || !status
          || status === "todo" || status === "ready"
          || status === "running" || status === "blocked" || status === "triage") {
        if (_personMatches(t.assigned_to, meId)) n += 1;
      }
    }
    return n;
  }
  ...
  function renderNavItem(n, activePanel, collapsed, counts) {
    const countText = counts[n.id] || "";
    const isPlanBadge = n.id === "plan" && countText;
    return el("button", {
      type: "button",
      class: `nav-item ${n.id === activePanel ? "active" : ""}`,
      "data-panel": n.id,
      "aria-current": n.id === activePanel ? "page" : "false",
      "aria-label": isPlanBadge ? `${n.label}, ${countText} open chores` : n.label,
      onclick: () => showPanel(n.id),
      title: collapsed ? n.label : "",
    },
      el("div", { html: ICONS[n.icon] || "" }),
      !collapsed && el("span", { class: "nav-label" }, n.label),
      !collapsed && el("span", {
        class: isPlanBadge ? "num nav-badge" : "num",
        "aria-hidden": isPlanBadge ? "true" : "false",
      }, countText)
    );
  }
  ...
  const ICONS = {
    ...
    calendar: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M8 3v4M16 3v4M3 10h18"/></svg>`,
  ...
  function renderShell() {
    ...
    // family-bot-0c6: bottom tab bar must always show labels — never honor
    // desktop "collapsed" on ≤768 (collapse control is hidden on mobile).
    const mobileShell = typeof window !== "undefined"
      && window.matchMedia("(max-width: 768px)").matches;
    const collapsed = !mobileShell
      && localStorage.getItem("bernie-sidebar-collapsed") === "true";
    ...
    const counts = { today: "", home: "", network: "", plan: "" };
    if (D.rooms) {
      try {
    ...
    // family-bot-0c6: Plan nav badge = my incomplete chores (not agent jobs)
    try {
      const openChores = countMyOpenChores();
      counts.plan = openChores > 0 ? String(openChores) : "";
    } catch {}
        ...
        (() => {
          ...
          if (overflow.length) {
            ...
            items.push(el("button", {
              ...
              !collapsed && el("span", { class: "nav-label" }, "More")
            ));
          }
          return items;
        })()
      ),
      // family-bot-tsi / q69: Discord+model telemetry Admin-only (not parents/kids)
      !collapsed && (() => {
        const role = ((window.Me && window.Me.role) || "").toLowerCase();
        const isTrueAdmin = role === "admin";
        const fam = (window.Me && window.Me.family_name) || "";
        const famLabel = fam
          ? (String(fam).includes("Family") ? fam : `${fam} Family`)
          : "our home";
        if (isTrueAdmin) {
          return el("div", { class: "bot-status", "data-operator-chrome": "1" },
            el("div", { class: "row" },
              el("span", { class: "dot" }),
        ...
        return el("div", { class: "bot-status" },
          el("div", { class: "row" },
            el("span", { class: "dot" }),
            el("span", {}, famLabel)
          ),
          el("div", { class: "meta" }, "bernie is here · ask anytime")
        );
      })()
    );
  ...
  function flashBernie(msg) {
    const n = el("div", {
      role: "status",
      "aria-live": "polite",
      style: "position: fixed; top: 20px; left: 50%; transform: translateX(-50%); background: var(--bg-card-2); border: 1px solid var(--amber); color: var(--ink); padding: 10px 18px; border-radius: 999px; box-shadow: 0 4px 24px rgba(0,0,0,.4); font-family: var(--font-serif); font-style: italic; z-index: 500;",
    }, msg);
  ...
  /**
   * family-bot-wze: unlock no longer fans out all family APIs.
   * Kept as a thin alias so older call sites still resolve; panel data
   * loads via loadPanelData / showPanel only.
   */
  async function loadFamilyBootstrap(panelId) {
    await loadPanelData(normalizePanelId(panelId || _activePanel || "today"));
  }
  window.loadFamilyBootstrap = loadFamilyBootstrap;

  async function refreshHomeData() {
    // family-bot-wze: single dashboard fetch (embeds rooms/temps) — not N parallel
    const safe = (p) => p.catch(e => { console.error("Home API failed:", e); return null; });
    const dash = await safe(api("/api/home/dashboard"));
    let anyOk = false;
  ...
  async function refreshTodayData(opts = {}) {
    // family-bot-wze: cold path = /api/today only (≤1 panel call); rooms soft-fetched after
    // family-bot-julyaudit P0: soft-fetch tasks so Needs-you attention rail is not blind
    const safe = (p) => p.catch(e => { console.error("API call failed:", e); return null; });
    const cold = !!opts.cold || !!_unlockTrace;
    const today = await safe(api("/api/today"));
    if (today) Object.assign(D, today);
    if (!cold) {
      const [temps, rooms, tasks] = await Promise.all([
      ...
      // After first paint: tasks (attention rail) + rooms (quick lights) without blocking TTFP
      Promise.resolve().then(async () => {
        const needRooms = !D.rooms || !D.rooms.length;
        const [rooms, tasks] = await Promise.all([
          needRooms ? safe(api("/api/rooms")) : Promise.resolve(null),
          safe(api("/api/tasks?all_people=true")),
        ]);
        let dirty = false;
        if (rooms) { D.rooms = rooms; dirty = true; }
        if (tasks) { D.tasks = tasks; dirty = true; }
        if (dirty && _activePanel === "today" && window.renderToday) window.renderToday();
        updateSidebarCounts();
      });
    }
    // family-bot-46t.1: deferred re-fetch when presence was snapshot-light
    maybeSchedulePresenceEnrich("today", today);
  }
  // family-bot-5bu: Today weather-retry card calls this from v3_today.js
  window.refreshTodayData = refreshTodayData;

  /** One deferred re-fetch after first paint when presence is stale/light. */
  let _presenceEnrichTimer = null;
  let _presenceEnrichCooldown = 0;
  function _presenceLooksStale(payload) {
    if (!payload) return false;
    if (payload.presence_stale) return true;
    const list = payload.presence || (Array.isArray(payload) ? payload : null);
    if (!list) return false;
    return list.some(p => p && p.snapshot);
  }
  function maybeSchedulePresenceEnrich(source, payload) {
    if (!_presenceLooksStale(payload)) return;
    if (_presenceEnrichTimer) return;
    // Cap: at most one enrich pass every 25s (avoids loop if HA still cold)
    const now = Date.now();
    if (now - _presenceEnrichCooldown < 25_000) return;
    _presenceEnrichTimer = setTimeout(async () => {
      _presenceEnrichTimer = null;
      // family-bot-julyaudit P0: advance cooldown (was misnamed assignment; never stuck)
      _presenceEnrichCooldown = Date.now();
      try {
        if (source === "today" || _activePanel === "today") {
          const safe = (p) => p.catch(() => null);
          const today = await safe(api("/api/today"));
          if (today) {
            Object.assign(D, today);
            if (_activePanel === "today" && window.renderToday) window.renderToday();
          }
        }
        if (source === "people" || _activePanel === "people") {
          const safe = (p) => p.catch(() => null);
          const family = await safe(api("/api/family"));
          if (family) {
            D.family = family;
            if (_activePanel === "people" && window.renderFamily) window.renderFamily();
          }
        }
      } catch (e) {
        console.error("presence enrich failed:", e);
      }
    }, 700);
  }
  window.maybeSchedulePresenceEnrich = maybeSchedulePresenceEnrich;

  async function refreshAdminData(opts = {}) {
    // family-bot-wze: no unlock fan-out; skip network if already loaded unless force
    const forceNet = !!opts.forceNetwork;
    const safe = (p) => p.catch(e => { console.error("API call failed:", e); return null; });
    const needNetwork = forceNet || !D.network;
    const [settings, network, models, keys, activity, usage, notifLog] = await Promise.all([
      safe(api("/api/settings")),
      needNetwork ? safe(api("/api/network/devices")) : Promise.resolve(null),
  ...
  function updateSidebarCounts() {
    ...
    // family-bot-0c6 / d3t: keep Plan chore badge live after task WS updates
    const planNav = document.querySelector('.nav-item[data-panel="plan"] .num');
    if (planNav) {
      try {
        const openChores = countMyOpenChores();
        planNav.textContent = openChores > 0 ? String(openChores) : "";
        planNav.classList.toggle("nav-badge", openChores > 0);
        planNav.setAttribute("aria-hidden", openChores > 0 ? "true" : "false");
        const btn = planNav.closest(".nav-item");
        if (btn) {
          btn.setAttribute(
            "aria-label",
            openChores > 0 ? `Plan, ${openChores} open chores` : "Plan"
          );
        }
      } catch (_) {}
    }
  ...
  // family-bot-o5j: coalesce presence WS ticks into one paint
  let _presenceRenderTimer = null;
  function schedulePresenceRender() {
    if (_presenceRenderTimer) return;
    _presenceRenderTimer = setTimeout(() => {
      _presenceRenderTimer = null;
      updateSidebarCounts();
      if (_activePanel === "today" && window.renderToday) window.renderToday();
      // o5j.1: only re-paint Family when we have local family rows to show
      if (_activePanel === "people" && window.renderFamily && Array.isArray(D.family) && D.family.length) {
        window.renderFamily();
      }
    }, 250);
  }
  ...
  function connectWS() {
    ...
    ws.onmessage = ({ data }) => {
      ...
      // family-bot-o5j / o5j.1: patch presence + family in place; debounce re-render
      if (e.type === "presence.update") {
        const matchId = (x) =>
          x && (x.id === e.id || x.who === e.id ||
            String(x.id || "").replace(/^person:/, "") === String(e.id || "").replace(/^person:/, "") ||
            String(x.who || "").toLowerCase() === String(e.id || "").toLowerCase());
        const applyPatch = (p) => {
          if (!p) return;
          if (e.home != null) p.home = e.home;
          if (e.sub != null) p.sub = e.sub;
          if (e.last_seen_ts) p.last_seen_ts = e.last_seen_ts;
          if (e.last_seen != null) p.last_seen = e.last_seen;
          if (e.status_label != null) p.status_label = e.status_label;
          if (e.snapshot === false) p.snapshot = false;
          if (e.battery != null) p.battery = e.battery;
          if (e.wifi != null) p.wifi = e.wifi;
          if (e.essid != null) p.essid = e.essid;
          if (e.departing != null) p.departing = e.departing;
          if (e.gps) p.gps = e.gps;
        };
        if (Array.isArray(D.presence)) {
          const p = D.presence.find(matchId);
          if (p) applyPatch(p);
          else if (e.id) {
            D.presence.push({
              id: e.id, who: e.id, home: e.home, sub: e.sub,
              last_seen_ts: e.last_seen_ts, snapshot: e.snapshot,
            });
          }
        }
        // o5j.1: Family panel reads D.family — keep it in sync or skip renderFamily
        if (Array.isArray(D.family)) {
          const fp = D.family.find(matchId);
          if (fp) applyPatch(fp);
        }
        schedulePresenceRender();
      }
      // family-bot-46t.1: server filled full presence cache — re-fetch active panel
      if (e.type === "presence.enriched") {
        if (_activePanel === "today") {
          refreshTodayData().then(() => {
            if (window.renderToday) window.renderToday();
          }).catch(() => {});
        } else if (_activePanel === "people") {
          api("/api/family").then(family => {
            if (family) {
              D.family = family;
      ...
      if ((e.type === "light.state" || e.type === "light.update") && D.rooms) {
        ...
        if (lid) {
          ...
          if (light) {
            ...
            // family-bot-hcd: surgical light patch; full Home only if tile missing
            if (_activePanel === "home") {
              try {
                if (!(window.patchHomeLight && window.patchHomeLight(lid, e.on))) {
                  if (window.renderHome) window.renderHome(true);
                }
              } catch (_) {
                if (window.renderHome) window.renderHome(true);
            ...
            // family-bot-o5j residual: debounce Today light re-paint with presence
            if (_activePanel === "today") schedulePresenceRender();
            updateSidebarCounts();
          }
        }
      }
      if (e.type === "task.update") {
        ...
        api("/api/tasks?all_people=true").then(d => {
          if (d) {
            ...
            // family-bot-3rl: refresh Needs-you approvals count on Today
            if (_activePanel === "today" && window.renderToday) window.renderToday();
            updateSidebarCounts();
          }
        }).catch(() => {});
  ...
  function applyBranding() {
    const fam = (window.Me && window.Me.family_name) || "Family";
  ...
  async function checkAuth() {
    ...
    const unlock = async () => {
      ...
      // family-bot-wze measure: expect health + active panel only (≤2 via api())
      beginUnlockTrace();
      await loadInitial();
    ...
    try {
      ...
      if (res.ok) {
        ...
        for (const u of users) {
          // family-bot-10y: list > listitem > button; accent ring; name not double-announced
          const accent = u.color || "var(--amber)";
          const btn = el("button", {
            type: "button",
            "aria-label": `Log in as ${u.name}`,
            style: `width: 80px; height: 80px; border-radius: 20px; border: 3px solid ${accent}; background: var(--bg-card-2); color: ${accent}; font-family: var(--font-serif); font-size: 32px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 12px rgba(0,0,0,.2); transition: transform 0.1s;`,
            onclick: () => {
              ...
              input.value = "";
              input.setAttribute("aria-label", `PIN for ${u.name}`);
              input.focus();
            }
          ...
          const wrapper = el("div", {
            ...
            btn,
            el("span", { "aria-hidden": "true", style: "font-size: 14px; color: var(--ink-2); font-weight: 500;" }, u.name)
          );
          grid.append(wrapper);
        }
      }
    ...
    backBtn.onclick = () => {
      ...
      err.textContent = "";
      selectedPerson = null;
      // family-bot-10y: restore focus to first person control
  ...
  async function showPanel(id) {
    ...
    // family-bot-u9m: stop camera poll + revoke blobs when leaving Security/cameras
    if (
      (prevPanel === "security" || prevPanel === "cameras") &&
      id !== "security" &&
      id !== "cameras" &&
      typeof window.v3CamerasLeave === "function"
    ) {
      window.v3CamerasLeave();
    }
    if (prevPanel === "calendar" && id !== "calendar" && typeof window.v3CalendarLeave === "function") {
      window.v3CalendarLeave();
    }
  ...
  async function loadPanelData(navId) {
    ...
    if (navId === "people") {
      ...
      if (family) {
        D.family = family;
        maybeSchedulePresenceEnrich("people", family);
      }
      return;
    }
    if (navId === "calendar") {
  ...
  async function loadInitial() {
    // family-bot-wze: health only on unlock — active panel loads in showPanel
    renderShell();
    showSkeletons();
    const safe = (p) => p.catch(e => { console.error("API call failed:", e); return null; });
    const health = await safe(api("/api/health"));
    if (health) D.health = health;
    renderShell();
    if (!_intervalsStarted) {
      _intervalsStarted = true;
      setInterval(() => {
        if (_activePanel !== "home") return;
        if (window.refreshTemps) window.refreshTemps();
      }, 300_000);
      setInterval(() => {
  ...
  window.showPanel = showPanel;
    ...
    // family-bot-rgy: only re-render Today when that panel is active
    setInterval(() => {
      if (_activePanel !== "today") return;
      if (D.presence && window.renderToday) window.renderToday();
    }, 60_000);
  });
})();
