(function() {
  ...
  // family-bot-co8 residual: defaults + Me.person_colors from web_ui_prefs /api/me
  ...
  function personAccent(who) {
    ...
    // also allow canonical/display keys as configured
    for (const [k, v] of Object.entries(fromMe)) {
      if (String(k).toLowerCase() === key) return v;
    }
    return PERSON_ACCENTS_DEFAULT[key] || 'oklch(0.72 0.14 50)';
  }
  ...
  async function renderFamily() {
    ...
    const draw = () => {
      // family-bot-hcd: tear down Leaflet maps before wiping DOM
      root.querySelectorAll("[id^='map-']").forEach(node => {
        if (node._bernieMap) {
          try { node._bernieMap.remove(); } catch (_) {}
          node._bernieMap = null;
        }
      });
      root.innerHTML = "";
    ...
    const buildPersonCard = (p) => {
      const isHome = p.home;
      const accent = personAccent(p.who);
      const isOpen = _openMemoryId === p.who;
      // family-bot-igj: orbs first; map/last-seen expand; memory/email under More for parents
      const isKid = typeof window.isKidUser === "function" && window.isKidUser();
      const canOps = !isKid && typeof window.isAdminUser === "function" && window.isAdminUser();
      const role = String(p.role || "").toLowerCase();
      const isParentLike = role === "admin" || role === "parents" || role === "parent";
      // Kids: no full GPS of parents by default (constellation redesign stays deferred)
      const showGps = !!(p.gps && !(isKid && isParentLike));

      const mapBlock = showGps ? (() => {
            const mapId = `map-${p.who}-${Date.now()}`;
            const wrapper = el("div", { style: "margin-top: 10px; background: var(--bg-card-2); border-radius: var(--r-md); border: 1px solid var(--stroke); overflow: hidden" },
              el("div", { class: "row between", style: "padding: 10px 12px 6px; align-items: center" },
                el("div", { class: "t-eyebrow", style: "font-size: 10px; opacity: 0.6" }, "LIVE LOCATION"),
                el("a", {
              ...
              el("div", { style: "padding: 8px 12px 10px" },
                ...
                p.address ? el("div", { style: "font-size: 12px; color: var(--ink-3); margin-top: 3px;" }, p.address) : null
              )
            );
            // family-bot-wze/hcd: lazy Leaflet; destroy prior map on this node if any
            requestAnimationFrame(() => {
              setTimeout(async () => {
                const container = document.getElementById(mapId);
                if (!container) return;
                try {
                  if (typeof window.ensureLeaflet === "function") await window.ensureLeaflet();
                } catch (_) { return; }
                if (!window.L) return;
                if (container._leaflet_id && container._bernieMap) {
                  try { container._bernieMap.remove(); } catch (_) {}
                }
      ...
      const detailBlock = el("details", { class: "family-card-details", style: "margin-top: 12px;", "data-family-details": "1" },
        el("summary", { style: "cursor: pointer; font-size: 12px; color: var(--ink-3); list-style: none;" },
          "Last seen · battery · map"
        ),
        el("div", { class: "row between", style: "margin-top: 10px; align-items: center" },
          ...
          el("button", {
            type: "button",
            class: "btn btn-ghost",
            style: "font-size: 12px; padding: 2px 8px; height: auto; min-height: 0",
            onclick: async (e) => {
              e.preventDefault();
              const btn = e.currentTarget;
      ...
      const moreOps = canOps ? el("details", { style: "margin-top: 12px;", "data-family-more": "1" },
        el("summary", { style: "cursor: pointer; font-size: 12px; color: var(--ink-3);" }, "More · ping · email · memory"),
        el("div", { class: "col", style: "gap: 10px; margin-top: 10px;" },
          ...
          el("button", {
            type: "button",
            class: "row between",
            style: "width: 100%; padding: 10px 0; background: transparent; border: 0; border-top: 1px solid var(--stroke); color: var(--ink-2); cursor: pointer; font-size: 12px; text-align: left;",
            onclick: () => toggleMemory(p.who)
          },
            el("span", {},
              "Memory · ",
              el("strong", { style: "color: var(--ink); font-weight: 600" },
                (_memoryCache[p.who]
      ...
      return el("div", { class: "card", style: "overflow: hidden", "data-family-orb": "1" },
        ...
        el("div", { style: "padding: 14px 16px 12px" },
          ...
          p.conflict_label ? el("div", {
            style: "margin-top: 10px; font-size: 12px; color: var(--warn); font-weight: 600; display: flex; align-items: center; gap: 4px;"
          },
