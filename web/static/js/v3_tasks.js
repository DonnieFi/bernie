...
(function () {
  ...
  function mapTask(r) {
    ...
    return {
      ...
      raw_status: r.status || r.kanban_status || "todo",
      title: r.title || "", details: r.details || "",
      assigned_to: norm(r.assigned_to) || null,
      assigned_by: norm(r.assigned_by) || null,
  ...
  /** family-bot-3rl: chores waiting on parent OK */
  function pendingApprovalTasks() {
    if (!isAdminUser()) return [];
    return TASKLIST().filter(t => {
      if (t.type && t.type !== "chore") return false;
      const done = t.status === "done" || t.raw_status === "done";
      if (!done) return false;
      if (t.approved_at) return false;
      // API can_approve when present; else parents see done chores needing approval
      if (t.can_approve) return true;
      return !!t.requires_approval;
    });
  }

  function approveStripHtml() {
    const pending = pendingApprovalTasks();
    if (!pending.length) return "";
    const rows = pending.slice(0, 8).map(t => {
      const who = t.assigned_to ? nameOf(t.assigned_to) : "someone";
      return `<div class="approve-row" data-approve-id="${t.id}">
        <span class="approve-title">${esc(t.title || "Chore")}</span>
        <span class="approve-who t-muted">${esc(who)}</span>
        <span class="spc"></span>
        <button type="button" class="act primary" data-approve-yes="${t.id}">Approve</button>
        <button type="button" class="act" data-approve-no="${t.id}">Needs work</button>
      </div>`;
    }).join("");
    return `<div class="approve-strip" id="plan-approve-strip" aria-label="Needs your OK">
      <div class="approve-head">
        <span class="card-tag"><span class="pip warn"></span> Needs your OK</span>
        <span class="t-mono" style="color:var(--ink-4);font-size:10px">${pending.length}</span>
      </div>
      <div class="approve-list">${rows}</div>
    </div>`;
  }

  // ── state ───────────────────────────────────────────────────────────────────
  const MODE_KEY = "bernie.tasks.mode";
  const LENS_KEY = "bernie.tasks.lens"; // family-bot-d3t: mine | household | board
  const S = {
    ...
    _defaultsReadyFor: null,
  };
  const getMode = () => {
    // family-bot-d3t: HUD is workshop-only — never for kids
    if (!isAdminUser()) return "calm";
    return localStorage.getItem(MODE_KEY) === "hud" ? "hud" : "calm";
  };
  function getLens() {
    try {
      const v = localStorage.getItem(LENS_KEY);
      if (v === "mine" || v === "household" || v === "board") return v;
    } catch (e) { /* ignore */ }
    return "mine";
  }
  function setLens(v) {
    if (v !== "mine" && v !== "household" && v !== "board") v = "mine";
    S.lens = v;
    try { localStorage.setItem(LENS_KEY, v); } catch (e) { /* ignore */ }
    applyLensFilters();
  }
  /** Map lens → person/type filters (family-bot-d3t). */
  function applyLensFilters() {
    const lens = S.lens || getLens();
    S.lens = lens;
    if (lens === "mine") {
      S.fPerson = me().id ? normPersonId(me().id) : S.fPerson;
      S.fType = "chore";
    } else if (lens === "household") {
      S.fPerson = null;
      S.fType = "chore";
    } else {
      // board = full agent/research kanban; clear type lock, keep person if set
      S.fType = null;
    }
  }
  function ensurePlanDefaults() {
    const meId = me().id || "";
    // Re-run when signed-in person changes (shared tablet).
    if (S._defaultsReadyFor === meId && meId) {
      if (!isAdminUser() && (S.lens || getLens()) === "board") {
        // Kids never inherit a parent-left Board lens (family-bot-d3t).
        setLens("mine");
      } else if ((S.lens || getLens()) === "mine" && meId && !S.fPerson) {
        S.fPerson = normPersonId(meId);
      }
      return;
    }
    let lens = getLens();
    if (!isAdminUser() && lens === "board") lens = "mine";
    S.lens = lens;
    try { localStorage.setItem(LENS_KEY, lens); } catch (e) { /* ignore */ }
    applyLensFilters();
    S._defaultsReadyFor = meId;
  }
  ...
  // ── card ────────────────────────────────────────────────────────────────────
  function canQuickComplete(t) {
    // family-bot-nn4: one-tap chore complete without drawer
    if (!t || t.type !== "chore") return false;
  ...
  function card(t, opts) {
    ...
    if (canQuickComplete(t)) {
      crow += `<button type="button" class="k-complete-btn" data-quick-done="${t.id}" ` +
        `aria-label="Mark done: ${esc(t.title)}" title="Mark done">✓</button>`;
    }
    let mid = "";
  ...
  function column(title, sub, count, accent, body, laneId) {
    ...
    return `<section class="col"><div class="col-h">${accent ? `<span class="dot" style="background:${accent}"></span>` : ""}
      ...
      <div class="col-body"${laneAttr}>${count ? body : `<div class="empty">Nothing here right now.</div>`}</div></section>`;
  }
  ...
  function topRow1() {
    ...
    const lens = S.lens || getLens();
    const lensSub = lens === "mine" ? "my chores"
      : lens === "household" ? "household chores"
      : "full board";
    const hudTitle = '<span class="mono" style="font-size:18px;color:var(--accent)">$</span> tasks.live';
    // family-bot-d3t / 0ty: Mine / Household primary; Board secondary (admin/parents only)
    const lensOpts = isAdminUser()
      ? [["mine", "Mine"], ["household", "Household"], ["board", "Board"]]
      : [["mine", "Mine"], ["household", "Household"]];
    const lensSeg = lensOpts
      .map(([v, l]) => `<button class="${lens === v ? "on" : ""}" data-lens="${v}">${l}</button>`).join("");
    return `<div class="ttl"><h1>${isHud ? hudTitle : "Plan"}</h1>
      <div class="sub">${isHud ? `system hud · <span style="color:var(--accent)">●</span> ${running} running · ${blocked} blocked` : `${lensSub} · ${calmCount} active`}</div></div>
      <div class="search">🔍<input id="tb-search" placeholder="search tasks" value="${esc(S.search)}"><span class="kbd">/</span></div>
      <span class="spacer"></span>
      <div class="seg plan-lens" title="Plan lens">${lensSeg}</div>
      <div class="seg plan-view-seg">${[["board", "Columns"], ["month", "Month"], ["roster", "Roster"]].map(([v, l]) => `<button class="${S.view === v ? "on" : ""}" data-view="${v}">${l}</button>`).join("")}</div>
      ${isAdminUser()
        ? `<div class="seg" title="mode (h)">${[["calm", "◑ Calm"], ["hud", "▦ HUD"]].map(([v, l]) => `<button class="${getMode() === v ? "on" : ""}" data-mode="${v}">${l}${v === "hud" ? '<span class="kbd">h</span>' : ""}</button>`).join("")}</div>`
        : ""}
  ...
  function topRow2() {
    ...
    const lens = S.lens || getLens();
    // Person chips: hidden on Mine (locked to me); agents only on Board lens
    let people = "";
    if (lens !== "mine") {
      people = family().map(p => chip(S.fPerson === p.id, `data-person="${p.id}"`, `${avatar(p.id, 16)} ${esc(p.label)}`)).join("");
      if (lens === "board") {
        people += `<span class="vline"></span>` + AGENTS.map(a => chip(S.fPerson === a.id, `data-person="${a.id}"`, `${avatar(a.id, 16)} ${esc(a.label)}`)).join("");
      }
    }
    const types = lens === "board"
      ? uiTaskTypes().map(k => chip(S.fType === k, `data-type="${k}"`, badge(k, true))).join("")
      : chip(true, `data-type="chore"`, badge("chore", true));
    const mLabel = S.fMonth === "all" ? "All months" : monthLabel(S.fMonth, true);
    // "clear" only for filters the user actually set (not lens locks)
    const any = S.fMonth !== "all" || S.search
      || (lens === "board" && (S.fPerson || S.fType));
    return `<span class="flabel">filter</span><div class="frow">${people || `<span class="chip on">${esc((me().name || me().id || "me"))}</span>`}</div><span class="spacer"></span>
  ...
  // family-bot-hcd Plan slice: debounce filter paints; optional view-host only rebuild
  let _filterDebounce = null;
  function scheduleFilterRender() {
    clearTimeout(_filterDebounce);
    _filterDebounce = setTimeout(() => {
      _filterDebounce = null;
      window.renderTasks({ filterOnly: true });
    }, 120);
  }

  // ── main render ────────────────────────────────────────────────────────────
  window.renderTasks = function (opts) {
    const root = qEl("panel-tasks");
    if (!root) return;
    ensurePlanDefaults();
    // Re-apply mine person when Me loads after first paint
    if ((S.lens || getLens()) === "mine" && me().id && !S.fPerson) {
      S.fPerson = normPersonId(me().id);
    }
    root.dataset.mode = getMode();
    root.dataset.lens = S.lens || getLens();
    ...
    const viewHost = root.querySelector(".tb-view-host");
    // Surgical path: filter/search/lens — only swap board body (keeps drawer/focus)
    if (
      opts && opts.filterOnly && viewHost && root.querySelector(".tb-top") &&
      !S.quickOpen && !S.mMenu
    ) {
      viewHost.innerHTML = viewHtml;
      const row2 = root.querySelector(".tb-top .row2");
      if (row2) row2.innerHTML = topRow2();
      bindBoardDrag(root);
      if (S.focus) {
        const el = qEl(S.focus);
  ...
  async function doAction(id, act) {
    const t = TASKLIST().find(x => x.id === id);
    try {
      if (act === "approve") await api(`/api/tasks/${id}/approve`, { method: "POST", body: { approved: true } });
      else if (act === "reject") await api(`/api/tasks/${id}/approve`, { method: "POST", body: { approved: false } });
  ...
  function announceLive(msg) {
    let region = document.getElementById("plan-aria-live");
    if (!region) {
      region = document.createElement("div");
      region.id = "plan-aria-live";
    ...
    region.textContent = "";
    // Force re-announce identical messages
    setTimeout(() => { region.textContent = msg; }, 10);
  }
  const _quickDoneInflight = new Set();
  async function quickComplete(id) {
    // family-bot-nn4
    const key = String(id);
    if (_quickDoneInflight.has(key)) return;
    const t = TASKLIST().find(x => String(x.id) === key);
    if (!t || !canQuickComplete(t)) return;
    _quickDoneInflight.add(key);
    const btn = document.querySelector(`[data-quick-done="${id}"]`);
    if (btn) btn.disabled = true;
    try {
      await api(`/api/tasks/${id}/complete`, { method: "POST", body: {} });
      const name = (me().name || me().first_name || me().id || "you");
      const nice = String(name).charAt(0).toUpperCase() + String(name).slice(1);
      const msg = `Nice one, ${nice}.`;
      if (window.flashBernie) window.flashBernie(msg);
      announceLive(msg);
      await refresh();
      if (typeof window.updateSidebarCounts === "function") window.updateSidebarCounts();
    } catch (err) {
      if (window.flashBernie) window.flashBernie((err && err.message) || "Could not mark done");
      if (btn) btn.disabled = false;
    } finally {
      _quickDoneInflight.delete(key);
    }
  }
  ...
  document.addEventListener("click", e => {
    if (!inPanel(e)) return;
    const el = e.target.closest("[data-quick-done],[data-approve-yes],[data-approve-no],[data-view],[data-mode],[data-lens],[data-person],[data-type],[data-month],[data-month-toggle],[data-clear],[data-open],[data-close],[data-qa],[data-qa-type],[data-qa-assignee],[data-act],[data-cmt]");
    ...
    if (d.quickDone != null) {
      e.preventDefault();
      e.stopPropagation();
      quickComplete(Number(d.quickDone));
      return;
    }
    // family-bot-3rl: parent approve strip
    if (d.approveYes != null) {
      e.preventDefault();
      e.stopPropagation();
      doAction(Number(d.approveYes), "approve");
      return;
    }
    if (d.approveNo != null) {
      e.preventDefault();
      e.stopPropagation();
      doAction(Number(d.approveNo), "reject");
      return;
    }
    if (d.view != null) { S.view = d.view; S.focus = null; scheduleFilterRender(); }
    else if (d.lens != null) { setLens(d.lens); S.focus = null; scheduleFilterRender(); }
    else if (d.mode != null) { setMode(d.mode); }
    else if (d.person != null) {
      if ((S.lens || getLens()) === "mine") return;
      S.fPerson = S.fPerson === d.person ? null : d.person;
      S.focus = null;
      scheduleFilterRender();
    }
    else if (d.type != null && d.qaType == null) {
      if ((S.lens || getLens()) !== "board") return; // chore lock on mine/household
      S.fType = S.fType === d.type ? null : d.type;
      S.focus = null;
      scheduleFilterRender();
    }
    else if (el.hasAttribute("data-month-toggle")) { S.mMenu = !S.mMenu; window.renderTasks(); }
    else if (d.month != null) { S.fMonth = d.month; S.mMenu = false; S.focus = null; scheduleFilterRender(); }
    else if (el.hasAttribute("data-clear")) {
      S.fMonth = "all";
      S.search = "";
      S.focus = null;
      applyLensFilters(); // restore lens person/type locks
      scheduleFilterRender();
    }
  ...
  document.addEventListener("input", e => {
    ...
    if (e.target.id === "tb-search") {
      ...
      // family-bot-hcd: debounce search → view-host only paint
      _searchDebounce = setTimeout(() => window.renderTasks({ filterOnly: true }), 220);
    }
  ...
  document.addEventListener("keydown", e => {
    ...
    else if (e.key === "h" && isAdminUser()) { setMode(getMode() === "hud" ? "calm" : "hud"); }
