(function() {
  ...
  function providerForModel(id, data) {
    if (data?.provider) return data.provider;
    const prefix = (id || "").split("/", 1)[0];
    if (prefix === "codex") return "openai";
    if (["grok", "openrouter", "ollama"].includes(prefix)) return prefix;
    return (id || "").includes("claude") ? "anthropic" : "openrouter";
  }
  ...
  function buildDailyChart(days) {
    ...
    if (stackBy === 'provider') {
      const grouped = {};
      modelIds.forEach(id => {
        const sample = days.map(d => d.models[id]).find(Boolean);
        const provider = providerForModel(id, sample);
        (grouped[provider] ||= []).push(id);
      });
      const order = ["anthropic", "openai", "openrouter", "grok", "ollama", "litellm"];
      stacks = Object.entries(grouped)
        .map(([key, ids]) => ({
          key,
  ...
  function buildProviderCard(pData, days) {
    const accent = getModelColor(pData.provider);
    const modelIds = Array.from(new Set(days.flatMap(d => Object.keys(d.models))))
      .filter(id => days.some(d => providerForModel(id, d.models[id]) === pData.provider));
    ...
    const hasBudget = Number.isFinite(pData.balanceRemaining) && Number.isFinite(pData.budget);
    const balance = hasBudget ? pData.balanceRemaining : 0;
    const budget = hasBudget ? (pData.budget || balance || 0) : 0;
    const used = Math.max(0, budget - balance);
    const pctUsed = budget > 0 ? (used / budget) * 100 : 0;
    const runway = avg > 0 ? balance / avg : 0;

    const state = !hasBudget ? 'active' : balance <= 0 ? 'depleted' : (runway < 3 ? 'warn' : 'active');
    const budgetBlock = hasBudget
      ? el("div", { style: "margin-bottom: 20px;" },
          ...
          el("div", { class: "act-micro", style: "display: flex; justify-content: space-between; margin-top: 6px;" },
            ...
            el("span", {}, balance <= 0 ? "top-up needed" : `runway ${runway.toFixed(1)}d at current rate`)
          )
        )
      : el("div", { class: "act-micro", style: "margin-bottom: 20px;" },
          pData.auth === "oauth" ? "OAuth subscription · quota unavailable" : "Usage tracked locally"
        );

    return el("div", { class: "card card-pad" },
      el("div", { style: "display: flex; align-items: center; gap: 10px; margin-bottom: 14px;" },
        el("span", { class: "act-dot", style: `background: ${accent};` }),
        el("span", { class: "act-eyebrow" }, PROVIDER_LABELS[pData.provider] || pData.provider),
        StatusPill(state, !hasBudget ? (pData.auth === "oauth" ? "OAuth" : "tracked") : balance <= 0 ? 'depleted' : (runway < 3 ? runway.toFixed(1) + "d left" : 'active')),
      ...
      el("div", { style: "display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 20px;" },
        ...
        Sparkline(dailyCost, { stroke: accent, fill: 'rgba(77,122,154,0.14)', width: 160, height: 56 })
      ),
      budgetBlock,
